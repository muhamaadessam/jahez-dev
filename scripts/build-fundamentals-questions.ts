...
// 10 Topics for Software Fundamentals
// foop: OOP (10)
// fsld: SOLID (10)
// fpat: Design Patterns (10)
// fcln: Clean Code (10)
// fds: Data Structures (10)
// falg: Algorithms (10)
// fcnc: Concurrency (10)
// fsys: System Design (10)
// fnet: Networking (10)
// ftst: Testing (10)
...
// ==========================================
// 1. Object-Oriented Programming (foop)
// ==========================================
questions.push(
  q(
    "foop-001",
    "oop-four-pillars-encapsulation-abstraction",
    "fund-oop",
    "Junior",
    "ما هي الأركان الأربعة للبرمجة كائنية التوجه (OOP) وما الفرق بين التغليف (Encapsulation) والتجريد (Abstraction)؟",
    "الأركان الأربعة هي: التغليف، التجريد، الوراثة، وتعدد الأشكال. التغليف يركز على إخفاء الحالة الداخلية وحمايتها، بينما التجريد يركز على إخفاء تفاصيل التنفيذ المعقدة وإبراز الواجهة الضرورية فقط.",
    "التغليف (Encapsulation) يدمج البيانات والعمليات التي تعمل عليها في وحدة واحدة (Class) ويقيد الوصول المباشر إلى المتغيرات إلا عبر Getters/Setters أو دوال محددة لضمان تكامل الحالة. بينما التجريد (Abstraction) يُعنى بتبسيط الرؤية البرمجية للمستخدم (كالضغط على زر تشغيل السيارة دون الحاجة لفهم دورة الاحتراق الداخلي).",
    ...
    [{ title: "Refactoring Guru — OOP Basics", url: "https://refactoring.guru/design-patterns/oop" }]
  ),
  q(
    "foop-002",
    "oop-composition-over-inheritance-benefits",
    "fund-oop",
    "Junior",
    "لماذا ينصح مبدأ 'Composition over Inheritance' بتفضيل التركيب على الوراثة الكلاسيكية؟",
    "لأن الوراثة تنشئ ارتباطاً وثيقاً ثابتاً وقت الترجمة (Compile-time Coupling) وتؤدي لمشكلة الفئة الأساسية الهشة، بينما يتيح التركيب تبديل السلوك ديناميكياً وقت التشغيل وبمرونة عالية.",
    "تعتبر الوراثة علاقة 'is-a' تفضح تفاصيل الفئة الأب للابن (White-box reuse)، وأي تعديل في الأب قد يكسر الأبناء دون قصد (Fragile Base Class Problem). بينما يمثل التركيب علاقة 'has-a' (Black-box reuse) حيث يمتلك الكائن مراجع لكائنات أخرى تنفذ واجهات محددة، مما يسهل الاختبار واستبدال المكونات.",
    "// بدلاً من الوراثة الصلبة: class FlyingSwimmingDuck extends Duck\nclass Duck {\n  constructor(private flyBehavior: FlyBehavior, private swimBehavior: SwimBehavior) {}\n  performFly() { this.flyBehavior.fly(); }\n}",
    [
    ...
    [{ title: "Composition over inheritance - Wikipedia", url: "https://en.wikipedia.org/wiki/Composition_over_inheritance" }]
  ),
  q(
    "foop-003",
    "oop-polymorphism-static-vs-dynamic-dispatch",
    "fund-oop",
    "Mid",
    "ما هو تعدد الأشكال (Polymorphism) وما الفرق بين الإرسال الثابت (Static Dispatch) والإرسال الديناميكي (Dynamic Dispatch)؟",
    "تعدد الأشكال هو قدرة الكائنات المختلفة على الاستجابة لنفس الرسالة بطرق متباينة. الإرسال الثابت يُحسم وقت الترجمة (مثل Method Overloading)، بينما الديناميكي يُحسم وقت التشغيل عبر جداول الدوال الافتراضية (vtable).",
    "في اللغات ذات الكتابة الثابتة (مثل C++ و C# و Java)، يتم تنفيذ Dynamic Dispatch عن طريق Virtual Method Table (vtable). يحتوي كل كائن على مؤشر للـ vtable الخاصة بفئته، وعند استدعاء دالة افتراضية يتم قراءة عنوان الدالة في وقت التشغيل، مما يضيف كلفة معالجة طفيفة جداً لكنه يمنح مرونة معمارية هائلة.",
    ...
    [{ title: "Virtual method table - Wikipedia", url: "https://en.wikipedia.org/wiki/Virtual_method_table" }]
  ),
  q(
    "foop-004",
    "oop-diamond-problem-multiple-inheritance",
    "fund-oop",
    "Mid",
    "ما هي معضلة الألماس (The Diamond Problem) في الوراثة المتعددة وكيف تحلها اللغات الحديثة؟",
    "تحدث المعضلة عندما ترث فئة من فئتين تشتركان في فئة أب مشتركة، مما يسبب غموضاً حول أي نسخة من دوال أو خصائص الأب يجب أن ترثها الفئة الابنة.",
    "إذا كانت الكلاس A تمتلك دالة foo()، وقامت كل من B و C بوراثة A وتعديل foo()، ثم ورثت الفئة D من B و C معاً، فعند استدعاء D.foo() لا يستطيع المترجم تحديد أي دالة يقصدها المبرمج. منعت لغات مثل Java و C# الوراثة المتعددة للفئات واكتفت بتعدد واجهات (Interfaces)، بينما حلتها C++ بالوراثة الافتراضية (Virtual Inheritance) واستخدمت Python خوارزمية C3 Linearization (MRO).",
    ...
    [{ title: "Multiple inheritance and diamond problem - Wikipedia", url: "https://en.wikipedia.org/wiki/Multiple_inheritance" }]
  ),
  q(
    "foop-005",
    ...
    "fund-oop",
    "Junior",
    "ما الفرق المعماري الدقيق بين الفئة المجردة (Abstract Class) والواجهة (Interface)؟",
    "الفئة المجردة تمثل هوية جزئية مشتركة ('is-a') ويمكنها الاحتفاظ بحالة (State) وتنفيذ افتراضي للدوال، بينما الواجهة تمثل عقداً سلوكياً ('can-do') يحدد ماذا تفعل الكائنات دون احتفاظ بحالة الفئة.",
    "تُستخدم Abstract Classes عندما تشترك عدة فئات في كود تنفيذي مشترك وحالة محددة (Protected fields)، وتكون هناك علاقة وراثية عميقة. أما الواجهات (Interfaces) فتُستخدم لفصل واجهة الاستخدام تماماً عن التنفيذ وتحقيق فك الارتباط الكامل (Decoupling) وتمكين الاختبارات وعزل التبعيات.",
    ...
    [{ title: "Interfaces vs Abstract Classes - Microsoft Docs", url: "https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/tutorials/oop" }]
  ),
  q(
    "foop-006",
    "oop-law-of-demeter-principle-least-knowledge",
    "fund-oop",
    "Senior",
    "ما هو قانون ديميتر (Law of Demeter / Principle of Least Knowledge) وكيف تحمي الشفرة من سلاسل الاستدعاء الطويلة؟",
    "ينص القانون على أن المكون البرمجي يجب أن يتحدث فقط مع أصدقائه المقربين المباشرين، ولا ينبغي له معرفة الهيكل الداخلي للكائنات الفرعية التي يتعامل معها.",
    "تعتبر استدعاءات 'Train Wrecks' مثل a.getB().getC().getD().doSomething() انتهاكاً صارخاً لقانون ديميتر؛ لأنها تجعل الكائن A معتمداً بشدة على تفاصيل الهيكلة الداخلية لـ B و C و D. إذا تغيرت تركيبة C ينهار كود A. الحل هو تفويض المسؤولية (Tell, Don't Ask) بإضافة دالة وسيطة في B تقوم بالمطلوب مباشرة a.getB().doActionOnD().",
    "// انتهاك صارخ: a.getInvoice().getCustomer().getAddress().getCity();\n// متوافق مع قانون ديميتر:\nconst city = invoice.getCustomerCity();",
    [
    ...
    [{ title: "Law of Demeter - Wikipedia", url: "https://en.wikipedia.org/wiki/Law_of_Demeter" }]
  ),
  q(
    "foop-007",
    "oop-coupling-and-cohesion-metrics",
    "fund-oop",
    "Senior",
    "ما الفرق بين الاقتران (Coupling) والتماسك (Cohesion) ولماذا نسعى دائماً إلى 'High Cohesion, Low Coupling'؟",
    "التماسك يقيس مدى ترابط وتركيز المسؤوليات داخل الوحدة الواحدة، بينما الاقتران يقيس درجة اعتماد الوحدات المختلفة على بعضها البعض.",
    "التماسك العالي (High Cohesion) يعني أن الفئة تقوم بمهمة واحدة محددة وتخدم هدفاً موحداً، مما يجعلها سهلة الفهم والصيانة وإعادة الاستخدام. الاقتران المنخفض (Low Coupling) يعني أن تغيير فئة ما لن يجبرك على تعديل عشرات الفئات الأخرى. العمارة الممتازة تجمع المهام المتشابهة معاً وتفصل التبعيات عبر واجهات مجردة مستقرة.",
    "// فئة منخفضة التماسك وعالية الاقتران (God Object):\nclass OrderManager {\n  processOrder() { /*...*/ }\n  sendEmail() { /*...*/ }\n  renderHtmlInvoice() { /*...*/ }\n  executeSql() { /*...*/ }\n}",
    [
    ...
    [{ title: "Coupling and Cohesion - Martin Fowler", url: "https://martinfowler.com/articles/refactoring-2nd-ed.html" }]
  ),
  q(
    "foop-008",
    "oop-value-objects-vs-entities-ddd",
    "fund-oop",
    "Mid",
    "ما هو الفرق الجوهري بين كائن القيمة (Value Object) والكيان (Entity) في النمذجة كائنية التوجه؟",
    "الكيان يمتلك هوية مميزة فريدة (Identity / ID) تمتد عبر الزمن حتى لو تغيرت خصائصه، بينما يُعرّف كائن القيمة فقط بمحتوى خصائصه وهو غير قابل للتغيير (Immutable) ولا يمتلك ID.",
    "المستخدم (User) هو Entity؛ لأن تغيير اسمه أو بريده لا يجعله شخصاً آخر وله معرف فريد ثابت (User ID). بينما المبلغ المالي (Money) أو العنوان (Address) يمثلان Value Objects؛ ورقة 100 دولار تطابق أي ورقة 100 دولار أخرى في القيمة، وتغيير المبلغ يعني إنشاء كائن قيمة جديد تماماً. تدعم Value Objects تكامل الدومين وتمنع الأخطاء الشائعة بمقارنة القيم (Structural Equality).",
  ...
  q(
    "foop-009",
    "oop-anemic-vs-rich-domain-model",
    "fund-oop",
    "Senior",
    "قارن بين نموذج الدومين الفقير (Anemic Domain Model) ونموذج الدومين الثري (Rich Domain Model)؟",
    "النموذج الفقير يفصل البيانات في كائنات بلا سلوك (مجموعات Getters/Setters) ويضع المنطق في خدمات خارجية، بينما يدمج النموذج الثري البيانات والمنطق وقواعد العمل معاً داخل الكيان.",
    ...
    [{ title: "Martin Fowler — AnemicDomainModel", url: "https://martinfowler.com/bliki/AnemicDomainModel.html" }]
  ),
  q(
    "foop-010",
    "oop-method-overriding-covariance-contravariance",
    "fund-oop",
    "Senior",
    "ما هي مفاهيم الـ Covariance والـ Contravariance في بارامترات وقيم إرجاع الدوال عند عمل Override؟",
    "عند استبدال دالة في الفئة الابنة: يجب أن تكون قيمة الإرجاع متوافقة أو أكثر تخصيصاً (Covariant)، بينما يجب أن تكون معاملات الإدخال متوافقة أو أكثر عمومية (Contravariant).",
    "وفقاً لمبدأ الاستبدال، إذا كان الكود يتوقع دالة ترجع Animal، فإن الفئة المشتقة يمكنها إرجاع Dog (تخصيص للناتج - Covariance) لأن الكلب هو حيوان ولن ينكسر الكود الطالب. أما بالنسبة لمعاملات الدالة، فإذا كانت الفئة الأصلية تقبل Dog، فلا يجوز للابن تضييق المعامل ليشترط GermanShepherd فقط، بل يجب أن يقبل Dog أو أكثر عمومية Animal (Contravariance) لضمان عدم حدوث أخطاء استدعاء غير متوقعة.",
    ...
    [{ title: "Covariance and contravariance - Wikipedia", url: "https://en.wikipedia.org/wiki/Covariance_and_contravariance_(computer_science)" }]
  )
);

console.log(`Pushed ${questions.length} questions so far.`);

// ==========================================
// 2. SOLID Principles (fsld)
// ==========================================
questions.push(
  q(
    "fsld-001",
    "solid-single-responsibility-principle-definition",
    "fund-solid",
    "Junior",
    "ما هو المعنى الحقيقي لمبدأ المسؤولية الواحدة (SRP) وما هو المقياس لتحديد 'المسؤولية'؟",
    "ينص SRP على أن الوحدة البرمجية يجب أن تمتلك سبباً واحداً فقط للتغيير، والمقياس الحقيقي هو خدمة جهة فاعلة واحدة (Single Actor) في المنظومة.",
    "كما يوضح روبرت مارتن (Uncle Bob)، لا يعني SRP أن الدالة تفعل شيئاً واحداً فقط؛ بل يعني أن الفئة مسؤولة أمام شخص أو جهة واحدة (مثل المدير المالي، أو مدير العمليات، أو مسؤول قاعدة البيانات). إذا كانت فئة Employee تحتوي على calculatePay() (مسؤولية المحاسبة) و save() (مسؤولية البنية التحتية) و reportHours() (مسؤولية الموارد البشرية)، فإن أي تعديل في سياسة أحد الأطراف يهدد استقرار الفئة لدى الأطراف الأخرى.",
    "// انتهاك: فئة تجمع بين حساب الرواتب وحفظ السجلات في قاعدة البيانات\n// التصحيح: فصل المسؤوليات\nclass PayrollCalculator {\n  calculateNetSalary(emp: Employee): number { /*...*/ }\n}\nclass EmployeeRepository {\n  save(emp: Employee): void { /*...*/ }\n}",
    [
  ...
  q(
    "fsld-002",
    "solid-open-closed-principle-real-world",
    "fund-solid",
    "Mid",
    "كيف تطبق مبدأ الانفتاح والإغلاق (Open/Closed Principle) دون الوقوع في فخ التعقيد المفرط؟",
    "يكون الكود مفتوحاً للتوسيع (إضافة ميزات جديدة) ومغلقاً للتعديل (دون الحاجة لتغيير الكود المجرب مسبقاً) عبر الاعتماد على الواجهات والاستراتيجيات المجردة.",
    "الهدف هو إضافة ميزة جديدة بمجرد كتابة فئة جديدة تنفذ واجهة محددة دون فتح وتعديل ملفات الكود الحالية بسلاسل switch-case طويلة. ولكن يجب تطبيقه بحذر فقط عند محاور التغيير المؤكدة (Axes of Change)، وليس على كل سطر تحسباً لمستقبل وهمي (YAGNI).",
  ...
  q(
    ...
    [{ title: "Liskov substitution principle - Wikipedia", url: "https://en.wikipedia.org/wiki/Liskov_substitution_principle" }]
  ),
  q(
    "fsld-004",
    "solid-interface-segregation-fat-interfaces",
    "fund-solid",
    "Junior",
    "لماذا يحذر مبدأ فصل الواجهات (Interface Segregation Principle) من الواجهات الضخمة (Fat Interfaces)؟",
    "لأنه لا ينبغي إجبار أي عميل برمجي على الاعتماد على دوال أو واجهات لا يستخدمها، مما يمنع الاقتران غير الضروري وإعادة الترجمة غير المبررة.",
    "عندما تحتوي الواجهة على عشرات الدوال المتنوعة (God Interface)، فإن أي فئة تطبقها ستضطر لكتابة دوال فارغة أو رمي أخطاء للوظائف التي لا تلزمها، وأي تعديل على دالة واحدة في الواجهة يجبر جميع الفئات المطبقة لها على إعادة الترجمة والاختبار. الحل هو تقسيم الواجهة الضخمة إلى واجهات أصغر وأدق تركز على أدوار محددة (Role Interfaces).",
  ...
  q(
    ...
    [{ title: "Dependency inversion principle - Wikipedia", url: "https://en.wikipedia.org/wiki/Dependency_inversion_principle" }]
  ),
  q(
    "fsld-006",
    "solid-ioc-containers-and-dependency-injection",
    "fund-solid",
    "Mid",
    "ما هو الفرق بين نمط Service Locator وحاويات حقن التبعيات (IoC Containers) ولماذا يُعتبر الأول مضاد نمط أحياناً؟",
    "يعتمد Service Locator على استدعاء مركزي صريح من داخل الفئة لجلب التبعيات مما يخفيها، بينما يقوم DI بتمرير التبعيات علانية عبر الـ Constructor مما يجعل متطلبات الفئة واضحة وصريحة.",
    "يخفي Service Locator التبعيات الحقيقية للفئة؛ فعند النظر إلى منشئ الفئة يبدو فارغاً، لكن الكود ينفجر وقت التشغيل إذا لم يتم تسجيل خدمة معينة في السجل المركزي. كما أنه يصعب كتابة اختبارات الوحدة لأنه يتطلب إعداد الحالة العامة للـ Locator. في المقابل، يفرض Constructor Injection عقداً معلناً لا يمكن إنشاء الكائن بدونه.",
    "// مضاد نمط (Service Locator):\nclass InvoiceService {\n  private db = ServiceLocator.get<Database>(); // تبعية مخفية وغير مرئية من الخارج\n}\n// حقن التبعية الصريح:\nclass InvoiceService {\n  constructor(private db: Database) {} // واضحة تماماً وسهلة الـ Mock\n}",
    [
  ...
  q(
    "fsld-007",
    "solid-di-lifecycles-singleton-scoped-transient",
    "fund-solid",
    "Senior",
    "ما هي دورات حياة الكائنات في حاويات DI (Transient, Scoped, Singleton) وما هي مشكلة التبعية الأسيرة (Captive Dependency)؟",
    "الـ Transient ينشئ كائناً جديداً في كل طلب، والـ Scoped ينشئ كائناً واحداً لكل طلب HTTP/سياق عمل، والـ Singleton ينشئ نسخة واحدة لكل عمر التطبيق. وتحدث التبعية الأسيرة عندما يعتمد Singleton على Scoped/Transient.",
    "تعتبر التبعية الأسيرة (Captive Dependency) خطأ فادحاً يهدد أمان وتماسك البيانات: إذا تم حقن خدمة Scoped (مثل DbContext مخصص لطلب المستخدم الحالي) داخل خدمة من نوع Singleton، فسيتم أسر كائن قاعدة البيانات داخل الـ Singleton ولن يُحرر أبداً، مما يؤدي لتسريب الذاكرة (Memory Leak) ومشاركة سياق المستخدم الخاطئ بين طلبات مستخدمين مختلفين عبر الـ Threads!",
    "// خطأ كارثي (Captive Dependency):\n// خدمات Singleton طويلة العمر لا يجوز أن تحقن بداخلها خدمات Scoped أو Transient محملة بحالة طلب معينة",
    [
    ...
    [{ title: "Dependency injection in .NET - Lifecycles", url: "https://learn.microsoft.com/en-us/dotnet/core/extensions/dependency-injection#service-lifetimes" }]
  ),
  q(
    "fsld-008",
    "solid-over-engineering-and-speculative-generality",
    "fund-solid",
    "Senior",
    "متى يصبح تطبيق مبادئ SOLID مفرطاً وهندسة زائدة (Over-engineering) وما هي علامات رائحة الكود Speculative Generality؟",
    "يحدث الإفراط عندما تُبنى طبقات تجريد وواجهات وسيطة لميزات غير مطلوبة بدعوى الاستعداد للمستقبل، مما يضاعف تكلفة قراءة الشفرة وصيانتها دون أي عائد حقيقي.",
    "تظهر رائحة 'التعميم التخميني' عندما تجد واجهات برمجية لا تملك سوى تنفيذ واحد فقط، وفئات محولة (Adapters) لوظائف لن تتغير قط، ومرور المعاملات عبر خمس طبقات وسيطة لا تفعل شيئاً سوى إعادة التمرير. يجب تطبيق SOLID كعلاج لضغط التغيير الواقعي والتعقيد المتزايد، وليس كقالب إلزامي على كل سطر في تطبيق بسيط.",
    "// مثال هندسة مفرطة: إضافة 4 ملفات interface و factory لجمع رقمين!\n// القاعدة: ابدأ بأبسط حل يعمل، ثم أعد الهيكلة (Refactor) بمجرد ظهور متطلب التغيير الثاني.",
    [
  ...
  q(
    ...
    [{ title: "Working Effectively with Legacy Code - Michael Feathers", url: "https://en.wikipedia.org/wiki/Michael_Feathers" }]
  ),
  q(
    "fsld-010",
    "solid-dry-vs-srp-accidental-duplication",
    "fund-solid",
    "Mid",
    "ما الفرق بين التكرار الحقيقي (True Duplication) والتكرار العرضي (Accidental Duplication) ومتى يكون تكرار الكود أفضل من التجريد الخاطئ؟",
    "التكرار الحقيقي يحدث عندما يتطابق المنطق والسبب في التغيير، بينما التكرار العرضي هو تشابه في الشكل السطحي لكود يخدم جهتين فاعلتين مختلفتين ستتغير متطلباتهما بشكل مستقل مستقبلاً.",
    "إذا كان كود حساب خصم المتجر الإلكتروني يشبه سطرين من كود تقرير الضرائب، فدمجهما في دالة واحدة انتهاك لمبدأ SRP باسم DRY! مستقبلاً عندما تتغير قوانين الضرائب، سيعدل المطور الدالة فينكسر حساب المتجر دون قصد. التكرار العرضي أقل تكلفة بكثير من التجريد الخاطئ (The Wrong Abstraction is far worse than Duplication).",
    "// الأفضل إبقاء الكودين منفصلين إذا كان أحدهما لعملاء التجزئة والآخر للمؤسسات، لأن أسباب تغييرهما متباينة تماماً.",
    [
...
// ==========================================
// 3. Design Patterns (fpat)
// ==========================================
questions.push(
  q(
    "fpat-001",
    "design-patterns-creational-structural-behavioral-overview",
    "fund-patterns",
    "Junior",
    "ما هي التصنيفات الثلاثة الرئيسية لأنماط التصميم (Design Patterns) وما هو الغرض الأساسي لكل فئة؟",
    "التصنيفات هي: الإنشائية (Creational) لإدارة إنشاء الكائنات، الهيكلية (Structural) لتركيب وتنظيم العلاقات بين الفئات، والسلوكية (Behavioral) لتنظيم التواصل وتوزيع المسؤوليات.",
    "أنماط التصميم هي حلول مجربة وموثقة لمشاكل هندسية شائعة. الأنماط الإنشائية (مثل Factory و Singleton) تفصل عملية إنشاء الكائنات عن استخدامها. الأنماط الهيكلية (مثل Adapter و Decorator) توضح كيفية دمج الكائنات المختلفة لتكوين هياكل أكبر وأكثر مرونة. والأنماط السلوكية (مثل Strategy و Observer) تحدد كيفية تدفق البيانات والرسائل وتوزيع الأدوار بين الكائنات بسلاسة.",
  ...
  q(
    ...
    "fund-patterns",
    "Mid",
    "ما هو الفرق الجوهري بين نمط Factory Method ونمط Abstract Factory؟",
    "نمط Factory Method يعتمد على الوراثة لإنشاء نوع واحد من الكائنات عبر دالة مخصصة، بينما Abstract Factory يعتمد على التركيب لإنشاء عائلات كاملة من الكائنات المترابطة دون تحديد فئاتها الملموسة.",
    "في Factory Method، تمتلك الفئة دالة createProduct() تترك للفئات المشتقة حرية اختيار المنتج الملموس. أما في Abstract Factory، توجد واجهة برمجية كاملة تحتوي على عدة دوال إنشائية لإنشاء عائلة كاملة من المنتجات المتوافقة (مثل واجهة GUIFactory التي تنشئ Button و Checkbox لأنظمة مختلفة: WindowsFactory و MacFactory).",
    ...
    [{ title: "Abstract Factory Pattern - Refactoring Guru", url: "https://refactoring.guru/design-patterns/abstract-factory" }]
  ),
  q(
    "fpat-003",
    "design-patterns-singleton-pattern-pitfalls",
    "fund-patterns",
    "Mid",
    "لماذا يعتبر نمط السينجلتون (Singleton Pattern) مضاد نمط (Anti-pattern) في معظم تطبيقات المؤسسات الحديثة؟",
    "لأنه يدخل حالة عامة مشتركة (Global Mutable State) تخفي التبعيات وتجعل الاختبار المستقل مستحيلاً، فضلاً عن تعقيدات الأمان وتعدد الخيوط (Thread Safety).",
    "السينجلتون ينتهك مبدأ المسؤولية الواحدة (SRP) لأنه يدير دورة حياته بنفسه بالإضافة لأداء وظيفته الأساسية. في بيئات تعدد الخيوط والـ Unit Testing، تحتفظ الكائنات العامة بحالتها بين الاختبارات مما يؤدي لفشل عشوائي متسلسل وغير مفهوم. في النظم الحديثة، يتم الاعتماد على حاويات حقن التبعيات (DI Containers) لتسجيل الخدمات كـ Singleton Lifecycle مع إبقاء الكود نفسه عادياً وقابلاً للاختبار والـ Mock.",
    ...
    [{ title: "Singleton Pattern - Refactoring Guru", url: "https://refactoring.guru/design-patterns/singleton" }]
  ),
  q(
    "fpat-004",
    "design-patterns-strategy-pattern-vs-state-pattern",
    "fund-patterns",
    "Mid",
    "ما الفرق بين نمط الاستراتيجية (Strategy Pattern) ونمط الحالة (State Pattern) رغم تشابه هيكل الكود بينهما؟",
    "يكمن الفرق في النية البرمجية (Intent): الاستراتيجية تمثل خوارزميات مستقلة يختار العميل إحداها، بينما الحالة تمثل سلوكاً داخلياً يتغير تلقائياً مع تغير الحالة الداخلية للكائن دون علم العميل.",
    "في نمط Strategy، تكون الاستراتيجيات المختلفة غير واعية بوجود بعضها ولا تقرر التبديل بينها؛ بل يحدد المستدعي الخوارزمية المطلوبة (مثل استراتيجية الدفع: نقدي أو بطاقة). أما في نمط State، فإن حالات الكائن المختلفة تعرف بعضها وتتولى الانتقال التلقائي من حالة إلى أخرى (مثل دورة حياة الطلب: Draft -> Paid -> Shipped).",
    "// State: الحالة تنتقل داخلياً\nclass DraftState implements OrderState {\n  next(order: Order) { order.setState(new PaidState()); }\n}\n// Strategy: العميل يمرر الخوارزمية\nconst sorter = new Sorter(new QuickSortStrategy());",
    [
  ...
  q(
    "fpat-005",
    "design-patterns-observer-vs-pub-sub-architecture",
    "fund-patterns",
    "Senior",
    "ما الفرق المعماري بين نمط المراقب (Observer Pattern) ونمط الناشر والمشترك (Publish-Subscribe Pattern)؟",
    "في Observer، يعرف الموضوع (Subject) مراقبيه مباشرة ويحتفظ بقائمتهم في نفس مساحة الذاكرة، بينما في Pub-Sub، يفصل وسيط رسائل خارجي (Message Broker) بين الناشر والمشترك تماماً دون أي معرفة متبادلة.",
    "الـ Observer نمط تصميم متزامن داخل نفس التطبيق والذاكرة (In-process)، حيث يستدعي الـ Subject دالة update() في المراقبين مباشرة. أما الـ Pub-Sub فهو نمط معماري للأنظمة الموزعة؛ يرسل الناشر حدثاً إلى وسيط (مثل Kafka أو RabbitMQ أو Redis)، ويقوم الوسيط بتوصيله إلى مئات المشتركين بشكل غير متزامن تماماً، مع فصل كامل في المكان والزمان ومسار التنفيذ.",
    "// Observer: ارتباط مباشر في الذاكرة\nsubject.attach(observer);\nsubject.notify();\n\n// Pub-Sub: وسيط وسيط وسيط مستقل تماماً\nmessageBroker.publish('order.created', payload);",
    [
    ...
    [{ title: "Observer vs Publish-Subscribe - Microsoft Docs", url: "https://learn.microsoft.com/en-us/azure/architecture/patterns/publisher-subscriber" }]
  ),
  q(
    "fpat-006",
    "design-patterns-decorator-vs-adapter-vs-proxy",
    "fund-patterns",
    "Mid",
    "قارن بين الأنماط الهيكلية الثلاثة المتشابهة: Decorator و Adapter و Proxy من حيث الغرض؟",
    "الـ Adapter يغير واجهة كائن موجود ليتوافق مع واجهة أخرى، والـ Decorator يضيف وظائف ومسؤوليات جديدة لنفس الواجهة، والـ Proxy يتحكم في الوصول للكائن مع الاحتفاظ بنفس الواجهة تماماً.",
  ...
  q(
    "fpat-007",
    "design-patterns-builder-pattern-immutability",
    "fund-patterns",
    "Junior",
    "متى نستخدم نمط الباني (Builder Pattern) وكيف يحل مشكلة المنشئات التلسكوبية (Telescoping Constructors)؟",
    "يُستخدم Builder لبناء كائنات معقدة تحتوي على العديد من المعاملات الاختيارية خطوة بخطوة، وتجنب المنشئات الطويلة المربكة وضمان الحفاظ على عدم قابلية التعديل (Immutability).",
    "عندما يمتلك كائن 10 حقول، يضطر المطور لكتابة منشئات تلسكوبية: new User(name), new User(name, email), new User(name, email, age, phone, ...)، مما يسبب أخطاء فادحة في ترتيب المعاملات المتشابهة في النوع. يوفر نمط Builder واجهة متسلسلة (Fluent API) لتمرير المعاملات بوضوح، مع دالة build() نهائية تتحقق من صحة كافة الحقول وتنشئ كائناً نهائياً مجمداً وغير قابل للتعديل.",
    ...
    [{ title: "Builder Pattern - Refactoring Guru", url: "https://refactoring.guru/design-patterns/builder" }]
  ),
  q(
    "fpat-008",
    ...
    "fund-patterns",
    "Senior",
    "كيف يعمل نمط الأمر (Command Pattern) وكيف يتيح ميزة التراجع والإعادة (Undo/Redo) وجدولة العمليات؟",
    "يقوم Command بتغليف الطلب أو العملية بالكامل ككائن مستقل يحتوي على جميع البيانات اللازمة لتنفيذه أو التراجع عنه في وقت لاحق.",
    ...
    [{ title: "Command Pattern - Refactoring Guru", url: "https://refactoring.guru/design-patterns/command" }]
  ),
  q(
    "fpat-009",
    ...
    [{ title: "Martin Fowler — Unit of Work", url: "https://martinfowler.com/eaaCatalog/unitOfWork.html" }]
  ),
  q(
    "fpat-010",
    "design-patterns-chain-of-responsibility-middleware",
    "fund-patterns",
    "Mid",
    "كيف يعمل نمط سلسلة المسؤولية (Chain of Responsibility) وما علاقته بمعمارية الـ Middleware في خوادم الويب الحديثة؟",
    "يمرر النمط الطلب عبر سلسلة من المعالجات، حيث يقرر كل معالج إما معالجة الطلب وتمريره للمعالج التالي أو إيقاف السلسلة ورفض الطلب فوراً.",
    "هذا النمط هو الأساس المعماري لأنظمة الـ Middleware في Express و ASP.NET و Fastify و Laravel. عند وصول طلب HTTP، يمر أولاً بـ LoggingMiddleware، ثم CorsMiddleware، ثم AuthenticationMiddleware (الذي قد يقطع السلسلة ويرد بـ 401)، ثم RateLimitingMiddleware، وصولاً إلى معالج المسار النهائي (Route Handler). هذا يفصل اهتمامات البنية التحتية تماماً عن كود الخدمة.",
...
// ==========================================
// 4. Clean Code & Refactoring (fcln)
// ==========================================
questions.push(
  q(
    "fcln-001",
    "clean-code-meaningful-naming-cqs-principle",
    "fund-clean-code",
    "Junior",
    "ما هي قواعد التسمية المعبرة ومبدأ فصل الأوامر عن الاستعلامات (Command-Query Separation - CQS) في الشفرة النظيفة؟",
    "التسمية المعبرة توضح القصد والهدف والسياق دون الحاجة لتعليقات مفسرة، ومبدأ CQS يفرض أن الدالة إما أن تغير حالة النظام (Command) أو ترجع إجابة وبيانات (Query) ولكن ليس كلاهما معاً.",
    "تجنب الأسماء المبهمة مثل data أو info أو x. دالة get يجب ألا تعدل البيانات خلسة، ودالة set لا يجب أن ترجع نتائج معقدة. انتهاك CQS (مثل دالة loginUser() التي تعيد مصفوفة الأصدقاء وتعدل كلمة المرور) ينتج عنه آثار جانبية خفية (Side Effects) تجعل تتبع الأخطاء واختبار النظام كابوساً للمطورين.",
    "// انتهاك CQS: الدالة تقرأ وتعدل في نفس الوقت\nfunction getUserAndIncrementLoginCount(id: string): User { /*...*/ }\n\n// متوافق مع CQS: عمليتان منفصلتان\nfunction incrementLoginCount(id: string): void { /*...*/ }\nfunction getUser(id: string): User { /*...*/ }",
    [
    ...
    [{ title: "Martin Fowler — Command Query Separation", url: "https://martinfowler.com/bliki/CommandQuerySeparation.html" }]
  ),
  q(
    "fcln-002",
    "clean-code-code-smells-god-object-feature-envy",
    "fund-clean-code",
    "Mid",
    "ما هي روائح الكود (Code Smells) واشرح رائحتي 'God Object' و 'Feature Envy' وكيف تتخلص منهما؟",
    "روائح الكود هي مؤشرات سطحية في الشفرة تدل على وجود مشاكل تصميمية ومعمارية أعمق. 'God Object' هي فئة عملاقة تعرف وتفعل كل شيء، و'Feature Envy' تحدث عندما تكون الدالة مهتمة ببيانات فئة أخرى أكثر من بيانات فئتها.",
    "علاج God Object هو استخدام Extract Class وتقسيم المسؤوليات وفقاً لـ SRP. أما علاج Feature Envy فهو تطبيق أسلوب Move Method؛ إذا وجدت دالة في OrderService تستدعي عشرات الـ Getters من كائن Customer لحساب نقاط الولاء، فهذا يعني أن هذا المنطق ينتمي في الأصل داخل فئة Customer، فننقل الدالة إلى هناك ونستدعيها ببساطة.",
    "// Feature Envy: دالة تتدخل في بيانات كائن آخر وتطلب بياناته الداخلية بدلاً من تفويضه\nclass OrderPrinter {\n  printCustomer(c: Customer) {\n    // استدعاء 5 خصائص من العميل لحساب التنسيق\n  }\n}",
    [
  ...
  q(
    ...
    [{ title: "Cyclomatic complexity - Wikipedia", url: "https://en.wikipedia.org/wiki/Cyclomatic_complexity" }]
  ),
  q(
    "fcln-004",
    "clean-code-boy-scout-rule-technical-debt",
    "fund-clean-code",
    "Junior",
    "ما هي قاعدة الكشافة (The Boy Scout Rule) في إدارة وتخفيض الدين الفني (Technical Debt)؟",
    "تنص القاعدة على: 'اترك مكان التخييم أنظف مما وجدته'. أي أنه مع كل تعديل أو مهمة برمجية جديدة، يجب عليك تحسين جزء صغير من الكود المحيط قبل تسليم المهمة.",
    "الدين الفني يتراكم تدريجياً نتيجة الاختصارات السريعة لضغط المواعيد، وإذا تم تجاهله يتصلب النظام بالكامل. بدلاً من طلب أسابيع كاملة من الإدارة لإعادة كتابة الكود (وهو ما يُرفض عادة)، يضمن تطبيق قاعدة الكشافة تنظيفاً مستمراً وتدريجياً: تغيير اسم متغير غامض، استخراج دالة صغيرة، حذف كود ميت، أو إضافة اختبار لوحدة ناقصة.",
    "// سلوك الكشاف: قمت بإضافة حقل للمستخدم، وقمت أيضاً بحذف دالة مهجورة غير مستخدمة منذ عام كامل",
    [
  ...
  q(
    ...
    [{ title: "Fail-fast system - Wikipedia", url: "https://en.wikipedia.org/wiki/Fail-fast" }]
  ),
  q(
    "fcln-006",
    "clean-code-defensive-programming-vs-contract",
    "fund-clean-code",
    "Senior",
    "قارن بين البرمجة الدفاعية (Defensive Programming) ومبدأ التصميم بالتعاقد (Design by Contract - DbC)؟",
    "البرمجة الدفاعية تفترض أن جميع المدخلات غير موثوقة ويجب فحصها في كل دالة لتفادي الانهيار، بينما Design by Contract يحدد عقداً صريحاً بين المستدعي والمستدعى يتحمل كل طرف مسؤوليته فيه.",
    "الإفراط في البرمجة الدفاعية يؤدي لملء كل دالة داخلية بفحوصات if (arg == null) متكررة في كل طبقة حتى داخل الدومين الآمن. أما DbC فيحدد: الشروط المسبقة تقع على عاتق المستدعي (لو أرسلت قيمة سالبة فالخطأ خطؤك أنت)، والشروط اللاحقة يضمنها المنفذ. في الأنظمة الحديثة، تُستخدم البرمجة الدفاعية الصارمة على حدود النظام الخارجية (API Gateways, User Inputs)، بينما يعتمد الدومين الداخلي على DbC والأنواع القوية (Type Safety) لمنع التكرار.",
    "// حدود النظام (Defensive Validation):\nconst body = validateSchema(req.body); // دفاعي صارم على الحدود\n// منطق الدومين الداخلي (Contract):\nexecuteTransfer(source, target, body.amount); // يثق في صحة النوع المنقى",
    [
  ...
  q(
    "fcln-007",
    "clean-code-refactoring-techniques-extract-replace-conditional",
    "fund-clean-code",
    "Mid",
    "اشرح تقنيتي إعادة الهيكلة: Extract Method و Replace Conditional with Polymorphism ومتى تطبقهما؟",
    "Extract Method تستخرج قطعة كود مترابطة في دالة مستقلة باسم يوضح قصدها، و Replace Conditional with Polymorphism تستبدل كتل switch/case المعقدة بهيكل فئات ترث واجهة مشتركة.",
    ...
    [{ title: "Refactoring — Replace Conditional with Polymorphism", url: "https://refactoring.guru/replace-conditional-with-polymorphism" }]
  ),
  q(
    "fcln-008",
    "clean-code-code-reviews-effective-collaboration",
    "fund-clean-code",
    "Junior",
    "ما هي معايير مراجعة الكود الفعالة (Code Review) وكيف تفرق بين التنسيقات السطحية والنقاشات المعمارية؟",
    "المراجعة الفعالة تركز على التصميم، الأمان، الأداء، ومنطق العمل، وتترك التنسيقات السطحية (المسافات والفواصل) لأدوات الفحص والتهيئة الآلية (Linters & Formatters).",
    "يجب ألا تستهلك مراجعة الكود في نقاشات عقيمة حول موضع القوس أو أسماء المتغيرات المتطابقة مع معايير الشركة؛ فهذه مهمة Pre-commit hooks و Linters. يجب أن يركز المراجع على: هل يعالج الكود متطلبات العمل؟ هل توجد ثغرات أمنية كحقن SQL؟ هل توجد حالات سباق في التزامن؟ هل الاختبارات كافية وذات جودة؟ مع تقديم تعليقات بناءة ومحترمة بصيغة اقتراحات وليس أوامر.",
    "// تعليق مراجعة غير فعال: 'ضع مسافة هنا'\n// تعليق مراجعة فعال وبناء:\n// 'اقتراح: استدعاء دالة قاعدة البيانات داخل الـ Loop قد يسبب مشكلة N+1 في الإنتاج، هل يمكننا جلبها كدفعة واحدة عبر whereIn؟'",
    [
    ...
    [{ title: "Google Engineering Practices — Code Review Developer Guide", url: "https://en.wikipedia.org/wiki/Code_review" }]
  ),
  q(
    "fcln-009",
    "clean-code-primitive-obsession-solution",
    "fund-clean-code",
    "Mid",
    "ما هي رائحة الكود المسماة هوس الأنواع الأولية (Primitive Obsession) وكيف يحلها إنشاء Value Objects؟",
    "تحدث عندما يتم استخدام الأنواع الأولية البسيطة (مثل string, int, double) لتمثيل مفاهيم دومين معقدة تحتوي على قواعد عمل محددة (مثل البريد، الهاتف، العملة، ورقم الهوية).",
    "تمثيل رقم الهاتف كـ string يعني أنه يمكن تمرير أي نص عشوائي للدالة ('abc123')، مما يجبر كل دالة في النظام على تكرار التحقق من صحة الرقم عبر Regex. الحل هو إنشاء كائن قيمة مخصص مثل PhoneNumber أو Email؛ يقوم الـ Constructor بالتحقق من القواعد مرة واحدة فقط عند الإنشاء، مما يضمن أن أي كائن موجود في النظام صالح بنسبة 100% ويمنع تمرير بيانات فاسدة للمنطق الداخلي.",
    ...
    [{ title: "Primitive Obsession - Refactoring Guru", url: "https://refactoring.guru/smells/primitive-obsession" }]
  ),
  q(
    "fcln-010",
    "clean-code-yagni-kiss-minimalist-architecture",
    "fund-clean-code",
    "Junior",
    "ما هما مبدآ KISS و YAGNI ولماذا يعتبران أهم صمام أمان ضد تعقيد وتضخم المشاريع؟",
    "KISS ينص على 'اجعل الأمر بسيطاً للغاية' وتجنب التعقيد، و YAGNI ينص على 'لن تحتاجه على الإطلاق' ويحظر كتابة أي وظيفة أو بنية لم تطلبها المتطلبات الحالية فوراً.",
    "المشاريع لا تفشل بسبب نقص الميزات المستقبلية، بل تنهار تحت وطأة التعقيد الإدراكي وصعوبة صيانة آلاف الأسطر المكتوبة تحسباً لاحتمالات نادراً ما تحدث. كل سطر كود تكتبه يمثل التزاماً مستقبلياً بالصيانة والاختبار وإصلاح الأخطاء. المطور المحترف هو من يحل المشكلة المعقدة بأبسط كود ممكن ومفهوم، ويرفض بناء عوالم افتراضية قبل أوانها.",
    "// YAGNI: العميل طلب حفظ المستخدمين في ملف محلي بسيط، فلا تقم ببناء بنية Microservices موزعة مع Kafka و Redis!",
    [
...
// ==========================================
// 5. Data Structures (fds)
// ==========================================
questions.push(
  q(
    "fds-001",
    "data-structures-arrays-vs-linked-lists-cache-locality",
    "fund-data-structures",
    "Junior",
    "لماذا تتفوق المصفوفات (Arrays) على القوائم المترابطة (Linked Lists) في سرعة التكرار حتى لو كانت بنفس التعقيد الزمني O(N)؟",
    "بسبب التموضع المكاني في الذاكرة (Spatial Locality) ومواءمتها مع خطوط التخزين المؤقت للمعالج (CPU Cache Lines)، بينما تتوزع عقد القائمة المترابطة عشوائياً في الـ Heap.",
    "تُخزن عناصر المصفوفة في كتل متجاورة فيزيائياً في الذاكرة (Contiguous Memory). عندما يقرأ المعالج عنصراً، يقوم تلقائياً بجلب الكتلة المحيطة بالكامل (Cache Line عادة 64 بايت) إلى الـ L1/L2 Cache، فتكون العناصر التالية جاهزة بسرعة نانو ثانية دون التوجه للـ RAM. في المقابل، كل عقدة (Node) في Linked List تُخصص في مكان عشوائي في الذاكرة وتتطلب مؤشراً إضافياً، مما يسبب إخفاقات مستمرة في التخزين المؤقت (Cache Misses).",
  ...
  q(
    "fds-002",
    "data-structures-hash-tables-collision-resolution",
    "fund-data-structures",
    "Mid",
    "كيف تعمل جداول التجزئة (Hash Tables) وما الفرق بين حل التصادمات عبر Chaining وعبر Open Addressing؟",
    "تقوم دالة التجزئة بتحويل المفتاح إلى مؤشر رقمي في مصفوفة داخلية لتحقيق وصول O(1). وعندما ينتج نفس المؤشر لمفتاحين مختلفين (Collision)، تُحل عبر ربط القوائم (Chaining) أو البحث في الفهارس الشاغرة (Open Addressing).",
    "في Separate Chaining، يحتوي كل موضع في الجدول على قائمة مترابطة أو شجرة بحث حمراء-سوداء تجمع العناصر المتصادمة في نفس الـ Bucket. في Open Addressing (مثل Linear Probing أو Quadratic Probing)، تظل كل العناصر داخل المصفوفة الرئيسية نفسها؛ وعند حدوث تصادم، يبحث الخوارزمي عن الخانة التالية الفارغة وفقاً لمعادلة فحص محددة. يحدد عامل التحميل (Load Factor) متى يجب مضاعفة حجم الجدول وإعادة التجزئة (Rehashing).",
  ...
  q(
    "fds-003",
    "data-structures-binary-search-trees-avl-red-black",
    "fund-data-structures",
    "Mid",
    "ما هي أشجار البحث الثنائية (BST) ولماذا نحتاج لأشجار متوازنة ذاتياً مثل AVL و Red-Black Trees؟",
    "تضمن شجرة BST أن الابن الأيسر أصغر من العقدة والأيمن أكبر، لكن إدخال عناصر مرتبة يحولها إلى خط مستقيم مائل O(N)، لذا تتدخل الأشجار المتوازنة عبر الدوران (Rotations) لضمان عمق O(log N).",
    "أشجار AVL تفرض توازناً صارماً جداً؛ حيث لا يزيد فارق الارتفاع بين أي شجرتين فرعيتين عن 1، مما يجعلها مثالية للبحث السريع المتكرر. أشجار Red-Black تستخدم توازناً أكثر تساهلاً يعتمد على قواعد تلوين العقد بالأحمر والأسود، مما يقلل عدد الدورات المطلوبة أثناء الإدخال والحذف، ولذلك تُستخدم كبنية افتراضية في مكتبات اللغات الكبرى مثل TreeMap في Java و std::map في C++.",
    "// شجرة متدهورة غير متوازنة (Worst Case):\n// 1 -> 2 -> 3 -> 4 (تتحول لقائمة خطية O(N))\n// شجرة متوازنة ذاتياً (Self-Balancing):\n//     [2]\n//    /   \\\n//  [1]   [3] (عمق لوغاريثمي O(log N))",
    [
    ...
    [{ title: "Red-black tree - Wikipedia", url: "https://en.wikipedia.org/wiki/Red%E2%80%93black_tree" }]
  ),
  q(
    "fds-004",
    "data-structures-stack-queue-monotonic-stack",
    "fund-data-structures",
    "Junior",
    "قارن بين الطابور (Queue) والمكدس (Stack) وما هي المكدسات الرتيبة (Monotonic Stacks)؟",
    "المكدس يعمل بنظام 'من يدخل آخراً يخرج أولاً' (LIFO)، والطابور بنظام 'من يدخل أولاً يخرج أولاً' (FIFO). والمكدس الرتيب يحافظ على ترتيب تصاعدي أو تنازلي صارم لعناصره لحل مسائل النطاقات بـ O(N).",
    "تُستخدم الـ Stacks في تتبع استدعاء الدوال (Call Stack)، التراجع (Undo)، والتحقق من الأقواس المتوازنة. وتُستخدم الـ Queues في جدولة المهام ومعالجة الطلبات بالترتيب. أما الـ Monotonic Stack فيُستخدم في مسائل الخوارزميات المتقدمة (مثل إيجاد أول عنصر أكبر Next Greater Element أو حساب مساحة المستطيلات في الهيستوجرام)، حيث يتم إخراج العناصر التي تخل بالترتيب الرتيب قبل الإضافة.",
    "// Stack: push(1), push(2) -> pop() يعيد 2\n// Queue: enqueue(1), enqueue(2) -> dequeue() يعيد 1",
    [
    ...
    [{ title: "Stack (abstract data type) - Wikipedia", url: "https://en.wikipedia.org/wiki/Stack_(abstract_data_type)" }]
  ),
  q(
    "fds-005",
    "data-structures-heaps-priority-queues-heapify",
    "fund-data-structures",
    "Mid",
    "كيف يعمل الكوم الثنائي (Binary Heap) وكيف تمثله مصفوفة بسيطة بدون مؤشرات عقد مع عملية Heapify؟",
    "الكوم الثنائي هو شجرة ثنائية شبه مكتملة تحقق خاصية الكوم (العنصر الأب أصغر من أبنائه في Min-Heap أو أكبر في Max-Heap)، ويُمثل بكفاءة في مصفوفة حيث أبناء العنصر عند المؤشر i يقعان عند 2i+1 و 2i+2.",
    "لأن الشجرة مكتملة دائماً، لا نحتاج لتخزين مؤشرات pointers في الذاكرة؛ فالأب عند index يمتلك ابنه الأيسر عند 2*i + 1 والأيمن عند 2*i + 2، والأب عند floor((i-1)/2). تتيح عملية Heapify تحويل مصفوفة عشوائية إلى Heap في زمن خطي مدهش O(N) بدلاً من O(N log N) عبر بناء المستويات من الأسفل للأعلى، بينما يستغرق استخراج القمة O(log N).",
    "// تمثيل المصفوفة: [Min, Child1, Child2, Grandchild1, ...]\n// Left child = 2*i + 1\n// Right child = 2*i + 2\n// Parent = (i - 1) / 2",
    [
    ...
    [{ title: "Binary heap - Wikipedia", url: "https://en.wikipedia.org/wiki/Binary_heap" }]
  ),
  q(
    "fds-006",
    "data-structures-graphs-adjacency-matrix-vs-list",
    "fund-data-structures",
    "Mid",
    "ما الفرق بين مصفوفة التجاور (Adjacency Matrix) وقائمة التجاور (Adjacency List) في تمثيل الرسوم البيانية (Graphs)؟",
    "مصفوفة التجاور تستخدم شبكة ثنائية الأبعاد V×V ممتازة للرسوم الكثيفة وتوفر فحص وجود ضلع بـ O(1)، بينما قائمة التجاور تخزن جيران كل رأس في قائمة موفرة للذاكرة O(V + E) ومثالية للرسوم المتناثرة.",
    "في معظم التطبيقات العملية (كالشبكات الاجتماعية وشبكات الطرق)، تكون الرسوم متناثرة (Sparse Graphs) حيث عدد الأضلاع E أقل بكثير من V^2؛ لذلك تستهلك مصفوفة التجاور ذاكرة هائلة O(V^2) معظمها أصفار. قائمة التجاور تحتفظ فقط بالعلاقات الفعلية، مما يتيح التكرار السريع على الجيران في خوارزميات BFS و DFS بزمن O(V + E).",
    "// مصفوفة التجاور (Matrix):\n//   A B C\n// A 0 1 0\n// B 1 0 1\n// قائمة التجاور (List):\n// A -> [B]\n// B -> [A, C]",
    [
    ...
    [{ title: "Adjacency list - Wikipedia", url: "https://en.wikipedia.org/wiki/Adjacency_list" }]
  ),
  q(
    "fds-007",
    "data-structures-trie-prefix-tree-autocomplete",
    "fund-data-structures",
    "Senior",
    "كيف تعمل شجرة البادئات (Trie / Prefix Tree) ولماذا تتفوق على جداول التجزئة في البحث بالبادئة والإكمال التلقائي؟",
    "تخزن شجرة Trie السلاسل النصية حرفاً بحرف بحيث تشترك الكلمات ذات البادئة المتطابقة في نفس المسار، مما يتيح البحث واسترجاع البادئات بزمن O(L) حيث L هو طول الكلمة مستقلاً تماماً عن عدد الكلمات المخزنة.",
    "في جداول التجزئة، يجب معرفة الكلمة بالكامل لحساب الـ Hash، ولا يمكنك البحث عن كل الكلمات التي تبدأ بـ 'car' إلا بالمرور على كل عناصر الجدول O(N). أما في Trie، فتصل إلى العقدة 'c' ثم 'a' ثم 'r' في 3 خطوات فقط، ومنها تستطيع استعراض الشجرة الفرعية بالكامل للحصول على كافة مقترحات الإكمال التلقائي (Autocomplete) فورياً.",
    "// Trie:\n//        (root)\n//          |\n//         [c]\n//          |\n//         [a]\n//        /   \\\n//      [r]   [t]\n//     (car) (cat)",
    [
  ...
  q(
    "fds-008",
    "data-structures-lru-cache-implementation-details",
    "fund-data-structures",
    "Senior",
    "كيف تصمم ذاكرة مؤقتة بنظام الأقل استخداماً مؤخراً (LRU Cache) تحقق عمليات Get و Put بزمن O(1) صارم؟",
    "عبر دمج هيكلين للبيانات معاً: جدول تجزئة (Hash Map) للبحث الفوري O(1)، وقائمة مترابطة مزدوجة (Doubly Linked List) لتتبع ترتيب الاستخدام وإضافة وحذف العناصر في O(1).",
    "يحتفظ الـ Hash Map بمفتاح العنصر وقيمة تشير مباشرة إلى عقدة في القائمة المترابطة المزدوجة. تحتوي القائمة على رأس وهمي (Head) وذيل وهمي (Tail). عند استدعاء get(key) أو تعديل قيمة، نحدد العقدة من الـ Map وننقلها إلى بداية القائمة (Most Recently Used). وعند امتلاء السعة واستدعاء put()، نقوم بحذف العقدة الموجودة قبل الذيل مباشرة (Least Recently Used) وحذفها من الـ Map في O(1) ثابت.",
    ...
    [{ title: "LRU cache implementation - Wikipedia", url: "https://en.wikipedia.org/wiki/Cache_replacement_policies#LRU" }]
  ),
  q(
    "fds-009",
    ...
    [{ title: "Segment tree - Wikipedia", url: "https://en.wikipedia.org/wiki/Segment_tree" }]
  ),
  q(
    "fds-010",
    "data-structures-stack-vs-heap-memory-layout",
    "fund-data-structures",
    "Junior",
    "ما الفرق الجوهري بين ذاكرة المكدس (Stack Memory) وذاكرة الكومة (Heap Memory) في إدارة موارد البرنامج؟",
    "الـ Stack هو مساحة ذاكرة خطية سريعة جداً تُدار تلقائياً بواسطة المعالج لتخزين المتغيرات المحلية وإطارات الدوال، بينما الـ Heap مساحة ديناميكية ضخمة لحجز كائنات متغيرة الحجم يديرها المبرمج أو جامع القمامة (GC).",
    "يتحرك مؤشر الـ Stack بمجرد زيادة أو إنقاص سجل المؤشر (Stack Pointer) مع دخول وخروج الدوال؛ تخصيصها لحظي وتحريرها تلقائي ومحمي من التجزئة، لكن حجمها محدود وأي تجاوز يسبب Stack Overflow. أما الـ Heap فيتطلب البحث عن كتلة حرة وحجزها (malloc/new)، وهي أبطأ بكثير وعرضة لتجزئة الذاكرة (Memory Fragmentation) وتتطلب إدارة واعية لمنع تسريب الذاكرة.",
    "// المتغيرات المحلية الصغيرة على الـ Stack:\nint count = 42;\n// الكائنات الديناميكية على الـ Heap مع مؤشر محلي على الـ Stack:\nUser* user = new User('Alice');",
    [
...
// ==========================================
// 6. Algorithms & Complexity (falg)
// ==========================================
questions.push(
  q(
    "falg-001",
    "algorithms-big-o-omega-theta-notations",
    "fund-algorithms",
    "Junior",
    "ما هو الفرق الدقيق بين ترميزات Big-O و Big-Omega و Big-Theta في تحليل تعقيد الخوارزميات؟",
    "ترميز Big-O يمثل الحد الأعلى المقارب للنمو (السيناريو الأسوأ Worst Case)، و Big-Omega يمثل الحد الأدنى (السيناريو الأفضل Best Case)، و Big-Theta يمثل حداً محكماً يطابق سلوك الخوارزمية في جميع الحالات.",
    "عندما نقول خوارزمية O(N^2)، فإننا نضمن رياضياً أن استهلاكها للوقت لن ينمو أسرع من ثابت مضروب في N^2 عندما تقترب المدخلات من اللانهاية. أما Big-Omega (Ω) فتحدد أقل زمن ممكن. وإذا كان أفضل وأسوأ سيناريو متطابقين في معدل النمو (مثل MergeSort التي تأخذ N log N في كل الأحوال)، فنصفها بأنها Θ(N log N). يركز مهندسو البرمجيات على Big-O لضمان عدم انهيار النظام تحت الحمل الأقصى.",
  ...
  q(
    "falg-002",
    "algorithms-quicksort-vs-mergesort-stability",
    "fund-algorithms",
    "Mid",
    "قارن بين خوارزميتي QuickSort و MergeSort من حيث التعقيد المكاني، استقرار الترتيب (Stability)، وأسوأ سيناريو؟",
    "ترتيب MergeSort مستقر (Stable) ويضمن دائماً O(N log N) لكنه يستهلك ذاكرة إضافية O(N)، بينما QuickSort يرتب في نفس المكان (In-place O(1) auxiliary space) وأسرع عملياً لكنه غير مستقر وأسوأ سيناريو له O(N^2).",
    ...
    "// MergeSort: يقسم المصفوفة إلى نصفين ويدمجهما بذاكرة إضافية\n// QuickSort: يختار Pivot ويقسم العناصر حوله في نفس المكان",
    [
  ...
  q(
    "falg-003",
    "algorithms-binary-search-bounds-space-reduction",
    "fund-algorithms",
    "Junior",
    "كيف تعمل خوارزمية البحث الثنائي (Binary Search) وما هي الحيل الرياضية لتفادي مشكلة Integer Overflow؟",
    "تعمل على مصفوفة مرتبة مسبقاً بتقسيم نطاق البحث إلى النصف في كل خطوة بزمن O(log N)، ولتفادي طفحان الأعداد الصحيحة يُحسب المنتصف عبر low + (high - low) / 2 بدلاً من (low + high) / 2.",
    "إذا كانت قيمتا low و high قريبتين من الحد الأقصى للأعداد الصحيحة (مثل 2^31 - 1)، فإن جمعهما (low + high) يسبب طفحاناً عددياً (Integer Overflow) يتحول إلى رقم سالب في لغات مثل Java و C++، مما يؤدي لرمي ArrayIndexOutOfBoundsException. يمكن أيضاً تطبيق البحث الثنائي ليس فقط على مصفوفات الأرقام، بل على فضاءات الإجابات المستمرة (Binary Search on Answer Space) لحل مسائل التحسين والحد الأدنى للحد الأقصى.",
    ...
    [{ title: "Binary search algorithm - Wikipedia", url: "https://en.wikipedia.org/wiki/Binary_search_algorithm" }]
  ),
  q(
    "falg-004",
    "algorithms-bfs-vs-dfs-applications",
    "fund-algorithms",
    "Mid",
    "قارن بين البحث بالعرض (BFS) والبحث بالعمق (DFS) في استكشاف الرسوم البيانية ومتى تختار كلاً منهما؟",
    "يستكشف BFS الجيران مستوى بمستوى باستخدام طابور (Queue) وهو مثالي لإيجاد أقصر مسار في الرسوم غير الموزونة، بينما يتعمق DFS لأقصى مسار ممكن باستخدام مكدس (Stack/Recursion) ومثالي لاكتشاف الدورات والمطابقة الطوبولوجية.",
    "يضمن BFS زيارة كافة العقد الواقعة على بُعد خطوة واحدة أولاً، ثم خطوتين، مما يجعله الخوارزمي الأسرع لإيجاد أقصر مسار في شبكات التواصل ومسارات المتاهات غير الموزونة، لكنه يستهلك ذاكرة أكبر لحفظ طبقة الجيران O(W). بينما يتعمق DFS مستكشفاً فرعاً حتى آخره، مما يجعله ممتازاً لحل مسائل المتاهات الكاملة، الترتيب الطوبولوجي (Topological Sort)، كشف الدورات (Cycle Detection)، وحساب المكونات المتصلة بقوة (Strongly Connected Components).",
  ...
  q(
    "falg-005",
    "algorithms-dijkstra-vs-bellman-ford-shortest-path",
    "fund-algorithms",
    "Senior",
    "متى تخفق خوارزمية ديكسترا (Dijkstra) في إيجاد أقصر مسار ولماذا نلجأ إلى خوارزمية بلمان-فورد (Bellman-Ford)؟",
    "تخفق ديكسترا عند وجود أضلاع ذات أوزان سالبة لأنها تفترض جشعاً (Greedy) أن المسار المحسوب لا يمكن أن يقصر مستقبلاً، بينما تعالج Bellman-Ford الأوزان السالبة وتكتشف الدورات السالبة (Negative Cycles).",
    "تستخدم ديكسترا طابور أولوية (Min-Priority Queue) وتصل لتعقيد O((V + E) log V). عند وجود ضلع سالب، قد تجد الخوارزمية مساراً يبدو أطول لكنه يصبح أقصر لاحقاً بفضل القيمة السالبة، مما يربك منطق التثبيت الجشع. خوارزمية Bellman-Ford تكرر عملية الاسترخاء (Relaxation) لجميع الأضلاع V-1 مرة بتعقيد O(V * E)، وإذا أمكن تقصير أي مسار في الخطوة V، فهذا دليل قاطع على وجود دورة سالبة تؤدي لانهيار مفهوم أقصر مسار (-Infinity).",
    "// Dijkstra: سريعة جداً وتتطلب أوزاناً موجبة فقط\n// Bellman-Ford: تدعم الأوزان السالبة وتكتشف فخاخ التكلفة اللانهائية",
    [
  ...
  q(
    "falg-006",
    "algorithms-two-pointers-and-sliding-window",
    "fund-algorithms",
    "Junior",
    "كيف تحول تقنيتا المؤشرين (Two Pointers) والنافذة المنزلقة (Sliding Window) المسائل التربيعية O(N^2) إلى خطية O(N)؟",
    "عبر الاحتفاظ بمؤشرين يتحركان في اتجاه محدد بدلاً من فحص كل الأزواج الممكنة بحلقتين متداخلتين، مما يضمن زيارة كل عنصر عدداً ثابتاً من المرات.",
    "في تقنية Two Pointers (مثل مسألة Two Sum على مصفوفة مرتبة)، نضع مؤشراً في البداية والآخر في النهاية، فإذا كان المجموع صغيراً نحرك الأيسر للأمام، وإذا كان كبيراً نحرك الأيمن للخلف بزمن O(N). وفي Sliding Window (مثل إيجاد أطول سلسلة فرعية بدون تكرار)، نوسع النافذة بزيادة المؤشر الأيمن، وعند اختراق الشرط نقوم بتقليصها بزيادة المؤشر الأيسر، محافظين على فحص النطاق المتحرك بزمن خطي ثابت.",
    ...
    [{ title: "Sliding window protocol and algorithms", url: "https://en.wikipedia.org/wiki/Sliding_window_protocol" }]
  ),
  q(
    "falg-007",
    ...
    "fund-algorithms",
    "Senior",
    "ما هي الشروط الأساسية لتطبيق البرمجة الديناميكية (DP) وما الفرق بين Memoization و Tabulation؟",
    "تتطلب DP شرطين: مشاكل فرعية متداخلة (Overlapping Subproblems) وبنية تحتية مثالية (Optimal Substructure). Memoization نهج من الأعلى للأسفل (Top-Down مع كاش)، و Tabulation نهج من الأسفل للأعلى (Bottom-Up مع جدول).",
    "البنية التحتية المثالية تعني أن الحل الأمثل للمسألة الكبيرة يمكن تركيبه من الحلول المثالية للمسائل الأصغر. والمشاكل المتداخلة تعني أن نفس الحسابات تتكرر مراراً (مثل شجرة متتالية فيبوناتشي). في Top-Down Memoization، نستخدم الاستدعاء الذاتي العادي ونخزن النتيجة في Map لمنع إعادة الحساب. في Bottom-Up Tabulation، نبدأ بحل الحالات الأساسية (Base Cases) ونملأ جدول مصفوفة خطوة بخطوة، مما يوفر مساحة إطارات الـ Call Stack ويتيح تحسين استهلاك الذاكرة (Space Optimization).",
    "// Tabulation مع تحسين الذاكرة لمسألة فيبوناتشي بزمن O(N) ومساحة O(1):\nlet prev2 = 0, prev1 = 1;\nfor (let i = 2; i <= n; i++) {\n  const curr = prev1 + prev2;\n  prev2 = prev1;\n  prev1 = curr;\n}",
    [
  ...
  q(
    "falg-008",
    "algorithms-greedy-choice-property-knapsack",
    "fund-algorithms",
    "Mid",
    "ما هي خاصية الاختيار الجشع (Greedy Choice Property) ولماذا تنجح الخوارزميات الجشعة في Fractional Knapsack وتفشل في 0/1 Knapsack؟",
    "خاصية الاختيار الجشع تعني أن اختيار أفضل حل محلي لحظي يقود بالضرورة للحل الأمثل الشامل، وتنجح عند إمكانية تجزئة العناصر وتفشل عند اشتراط أخذ العنصر كاملاً أو تركه.",
    "في مسألة Fractional Knapsack، يمكننا ترتيب العناصر حسب النسبة بين القيمة والوزن (Value/Weight Ratio) واختيار أعلى نسبة دائماً وأخذ أجزاء كسرية من العنصر الأخير، محققين الحل الأمثل بـ O(N log N). لكن في 0/1 Knapsack، قد يؤدي اختيار عنصر ذي كثافة قيمة عالية إلى إشغال مساحة تمنع وضع عنصرين آخرين مجموعهما أفضل، مما يجعل الأسلوب الجشع قاصراً ويجبرنا على اللجوء للبرمجة الديناميكية.",
    "// الأسلوب الجشع يختار الأفضل فوراً دون ندم أو تراجع:\nitems.sort((a, b) => (b.value / b.weight) - (a.value / a.weight));",
    [
  ...
  q(
    "falg-009",
    "algorithms-backtracking-pruning-n-queens",
    "fund-algorithms",
    "Senior",
    "كيف تعمل خوارزميات التراجع (Backtracking) وما هو دور التقليم (Pruning) في استكشاف شجرة فضاء الحالات لمسألة N-Queens؟",
    "يقوم Backtracking ببناء الحل تدريجياً، وبمجرد اكتشاف أن المسار الحالي ينتهك الشروط، يتراجع خطوة للوراء (Backtracks)؛ ويقوم التقليم (Pruning) بإلغاء فروع الشجرة المستحيلة مسبقاً لتوفير الوقت.",
    "في مسألة N-Queens، بدلاً من تجربة كافة التباديل البالغة N^N بأسلوب القوة الغاشمة (Brute Force)، نضع ملكة في الصف الأول، ثم نبحث في الصف التالي عن عمود غير مهدد. إذا وجدنا أن وضع الملكة في خانة معينة يهدد قطرياً أو عمودياً، نقوم بالتقليم (Pruning) ونتجاهل استكشاف كل الصفوف اللاحقة المتفرعة من هذا الموضع، ونعود للوراء لتغيير موقع الملكة السابقة. هذا يقلص فضاء البحث من أرقام فلكية إلى بضعة آلاف خطوة.",
  ...
  q(
    "falg-010",
    "algorithms-bit-manipulation-tricks-masks",
    "fund-algorithms",
    "Mid",
    "ما هي أهم حيل معالجة البتات (Bit Manipulation) مثل فحص القوى الثنائية وحساب عدد البتات وإيجاد العنصر الفريد عبر XOR؟",
    "العمليات الثنائية تنفذ على مستوى دوائر المعالج في دورة ساعة واحدة (Single CPU Cycle)؛ فحص قوى 2 يتم عبر (n & (n - 1)) == 0، وإيجاد العنصر الفريد الوحيد يتم بتطبيق XOR على جميع الأرقام.",
    "خواص XOR المذهلة: A ^ A = 0 و A ^ 0 = A، والعملية تبادلية وتجميعية. فإذا كانت مصفوفة تحتوي على كل الأرقام مكررة مرتين عدا رقم واحد، فإن تطبيق XOR التراكمي يلغي كل العناصر المكررة ويتبقى الرقم الفردي تلقائياً بزمن O(N) ومساحة O(1)! حيلة n & (n - 1) تقوم بحذف البت الأحادي الأيمن الأخير (Least Significant Bit)، وتُستخدم في خوارزمية Brian Kernighan لعد البتات المضاءة بزمن يتناسب مع عدد الآحاد فقط.",
    "// فحص هل الرقم قوة لـ 2 (Power of Two):\nconst isPowerOfTwo = (n > 0) && ((n & (n - 1)) === 0);\n\n// إيجاد العنصر الفريد الفردي:\nconst singleNumber = (nums: number[]) => nums.reduce((acc, num) => acc ^ num, 0);",
    [
    ...
    [{ title: "Bitwise operation - Wikipedia", url: "https://en.wikipedia.org/wiki/Bitwise_operation" }]
  )
);

// ==========================================
// 7. Concurrency & Multi-threading (fcnc)
// ==========================================
questions.push(
  q(
    "fcnc-001",
    "concurrency-threads-vs-processes-memory-layout",
    "fund-concurrency",
    "Junior",
    "ما هو الفرق الجوهري بين العمليات (Processes) وخيوط التنفيذ (Threads) وتكلفة التبديل السياقي (Context Switching)؟",
    "العملية تمثل برنامجاً مستقلاً يمتلك مساحة عنوان ذاكرة افتراضية معزولة خاصة به، بينما تتشارك الخيوط داخل نفس العملية نفس الذاكرة والملفات ومساحة الكومة (Heap) وتمتلك مكدساً مستقلاً فقط.",
    "تبديل السياق بين عمليتين (Process Context Switch) مكلف وبطيء جداً؛ لأنه يتطلب من نواة نظام التشغيل (Kernel) تبديل جداول الصفحات (Page Tables) وتفريغ الـ TLB (Translation Lookaside Buffer) ومسح خطوط كاش المعالج. في المقابل، التبديل بين خيوط نفس العملية (Thread Context Switch) أسرع بكثير لأن مساحة الذاكرة والـ Page Tables لا تتغير، لكن مشاركة الذاكرة تفتح الباب على مصراعيه لحالات السباق وفساد البيانات.",
    "// Process 1 (Memory Isolated) <--- IPC (Sockets/Pipes) ---> Process 2 (Memory Isolated)\n// Process 1:\n//   ├── Thread A (Own Stack) ──┐\n//   └── Thread B (Own Stack) ──┴──> Shared Heap & Open Files",
    [
  ...
  q(
    ...
    "fund-concurrency",
    "Junior",
    "ما هي حالة السباق (Race Condition) وما هو القسم الحرج (Critical Section) في البرمجة المتزامنة؟",
    "القسم الحرج هو جزء الشفرة الذي يصل إلى مورد مشترك قابل للتعديل، وحالة السباق تحدث عندما تتنافس خيوط متعددة للوصول والتعديل على هذا المورد في نفس الوقت دون تزامن مناسب.",
    "المثال الكلاسيكي هو عملية count++ البسيطة؛ على مستوى لغة الآلة تتكون من 3 تعليمات: (1) قراءة القيمة من الذاكرة إلى السجل (Read). (2) إضافة واحد (Increment). (3) كتابة القيمة في الذاكرة (Write). إذا دخل خيطان في نفس الوقت، قد يقرأ كلاهما القيمة 5 معاً، ويحسبان 6، ويكتبان 6 في الذاكرة بدلاً من 7! هذا الفقدان العشوائي للبيانات يمثل حالة سباق كارثية.",
    "// كود غير آمن متعدد الخيوط:\nint counter = 0;\nvoid increment() {\n  counter++; // غير ذرية (Not Atomic) وتسبب Race Condition\n}",
    [
  ...
  q(
    "fcnc-003",
    "concurrency-mutex-semaphore-spinlock-readwrite",
    "fund-concurrency",
    "Mid",
    "قارن بين أدوات المزامنة: Mutex و Semaphore و Spinlock و Read-Write Lock ومتى تختار كلاً منها؟",
    "الـ Mutex قفل متبادل يملكه خيط واحد فقط، والـ Semaphore إشارة تسمح بعدد N من الخيوط المتزامنة، والـ Spinlock يدور في حلقة مشغولة دون تعليق الخيط، والـ ReadWrite Lock يسمح بقراء متعددين أو كاتب واحد فقط.",
    ...
    [{ title: "Lock (computer science) - Wikipedia", url: "https://en.wikipedia.org/wiki/Lock_(computer_science)" }]
  ),
  q(
    "fcnc-004",
    "concurrency-deadlocks-coffman-conditions",
    "fund-concurrency",
    "Senior",
    "ما هي شروط كوفمان الأربعة (Coffman Conditions) لحدوث الجمود (Deadlock) وكيف تكسر أحدها لحل المشكلة؟",
    "شروط الجمود الأربعة هي: (1) الحصر المتبادل (Mutual Exclusion). (2) التعليق والانتظار (Hold and Wait). (3) عدم انتزاع القفل قسراً (No Preemption). (4) الانتظار الدائري (Circular Wait). وكسر أي شرط منها يمنع الجمود نهائياً.",
    "يحدث الجمود الكلاسيكي عندما يمسك الخيط 1 بالقفل A وينتظر القفل B، بينما يمسك الخيط 2 بالقفل B وينتظر القفل A؛ يتوقف النظام للأبد. أسهل وأقوى استراتيجية هندسية لكسر الجمود هي كسر شرط الانتظار الدائري عبر فرض ترتيب صارم لاكتساب الأقفال (Lock Ordering): يجب على جميع خيوط النظام دائماً حجز القفل A قبل القفل B وفقاً لترتيب هرمي ثابت وموحد.",
    "// كسر الجمود بفرض ترتيب القفل عالمياً:\nvoid transfer(Account from, Account to) {\n  Account first = from.id < to.id ? from : to;\n  Account second = from.id < to.id ? to : from;\n  synchronized(first) {\n    synchronized(second) {\n      // مستحيل حدوث Deadlock لأن الترتيب موحد لكل العمليات!\n    }\n  }\n}",
    [
  ...
  q(
    "fcnc-005",
    "concurrency-memory-visibility-volatile-barriers",
    "fund-concurrency",
    "Senior",
    "ما هي مشكلة رؤية الذاكرة (Memory Visibility) وما هو دور الكلمة المفتاحية volatile وحواجز الذاكرة (Memory Barriers)؟",
    "تقوم المعالجات بتخزين المتغيرات في كاش محلي (L1/L2) وإعادة ترتيب التعليمات، مما يجعل التعديلات التي يجريها خيط غير مرئية فوراً للخيوط الأخرى؛ وتضمن volatile القراءة والكتابة المباشرة في الذاكرة المشتركة.",
    "بدون إشارات مزامنة، قد يدخل الخيط A في حلقة while (!flag) {}. إذا عدل الخيط B قيمة flag إلى true، قد لا يرى الخيط A هذا التغيير أبداً لأن المعالج وضع flag في سجل محلي وافترض أنها لن تتغير من داخل الحلقة. كلمة volatile تخبر المترجم والمعالج بعدم إعادة ترتيب التعليمات المحيطة وتفرض حاجز ذاكرة (Memory Barrier / Fence) يجبر المعالج على تفريغ القيمة إلى الذاكرة الرئيسية وقراءتها منها دائماً.",
    "// يضمن رؤية المتغير فوراً عبر كافة النوى بدون كاش معزول:\nprivate volatile boolean isRunning = true;",
    [
    ...
    [{ title: "Volatile (computer programming) - Wikipedia", url: "https://en.wikipedia.org/wiki/Volatile_(computer_programming)" }]
  ),
  q(
    "fcnc-006",
    "concurrency-atomic-operations-compare-and-swap",
    "fund-concurrency",
    "Senior",
    "كيف تعمل خوارزميات المقارنة والتبديل (Compare-And-Swap - CAS) وبناء هياكل البيانات غير المقفلة (Lock-Free)؟",
    "تعتمد تعليمات CAS على تعليمة ذرية مدمجة بالمعالج تقارن محتوى الذاكرة بقيمة متوقعة، وإذا تطابقت تستبدلها بالقيمة الجديدة بنجاح؛ وإذا فشلت يعيد الخيط المحاولة دون حجز أي قفل.",
    ...
    "// مفهوم حلقة CAS الذرية بدون أقفال:\ndo {\n  current = value.get();\n  next = current + 1;\n} while (!value.compareAndSet(current, next));",
    [
  ...
  q(
    "fcnc-007",
    "concurrency-thread-pools-sizing-work-stealing",
    "fund-concurrency",
    "Mid",
    "كيف تدير أحواض الخيوط (Thread Pools) المهام وما هي معادلة الحجم المثالي للـ CPU-bound مقابل I/O-bound؟",
    "تدير أحواض الخيوط طابوراً من المهام مع عدد محدد من الخيوط الجاهزة وتمنع انهيار النظام؛ وحجم الحوض للعمليات الحسابية يساوي عدد النوى (N_cpu)، بينما لعمليات الإدخال/الإخراج يساوي N_cpu * (1 + Wait_Time / Compute_Time).",
    "إنشاء وتدمير خيط لكل طلب مستخدم يؤدي لانهيار السيرفر بسبب استهلاك الذاكرة. يحتفظ الـ Thread Pool بعدد ثابت من الخيوط العاملة التي تلتقط المهام من BlockingQueue. إذا كانت المهام CPU-bound (تشفير أو ضغط)، فزيادة الخيوط عن عدد النوى يضر الأداء بسبب الـ Context Switching. أما إذا كانت I/O-bound (انتظار قاعدة البيانات أو الشبكة حيث يقضي الخيط 90% من وقته في الانتظار)، فيمكن زيادة الخيوط لـ 10 إلى 50 ضعفاً لتشغيل النوى أثناء انتظار الآخرين.",
    "// ForkJoinPool تستخدم خوارزمية سرقة العمل (Work-Stealing):\n// عندما ينتهي خيط من طابوره الخاص، يسرق مهام من ذيل طوابير الخيوط المزدحمة لضمان أقصى كفاءة!",
    [
    ...
    [{ title: "Thread pool - Wikipedia", url: "https://en.wikipedia.org/wiki/Thread_pool" }]
  ),
  q(
    "fcnc-008",
    "concurrency-event-loop-vs-thread-per-request",
    "fund-concurrency",
    "Mid",
    "قارن بين نموذج الخيط لكل طلب (Thread-per-request) ونموذج حلقة الأحداث غير المحجوبة (Event Loop with Non-blocking I/O)؟",
    "نموذج الخيط لكل طلب يخصص خيطاً كاملاً لكل اتصال مستخدم ويحجزه طوال فترة الانتظار، بينما تعتمد حلقة الأحداث على خيط واحد يستمع لأحداث المقابس عبر آليات النواة (epoll/kqueue) دون حجب.",
    "في السيرفرات الكلاسيكية (مثل Apache Tomcat القديم)، حجز خيط لكل طلب يعني أنه عند وصول 10,000 اتصال متزامن يحتاج السيرفر لـ 10,000 خيط، مما يستهلك عدة غيغابايت من الذاكرة للمكدسات (Stacks) وينهار تحت وطأة الـ Context Switching (معضلة C10K). في المقابل، يعمل Node.js و Nginx و Netty بحلقة أحداث واحدة: يُسجل مقبس العميل مع النواة، وعند وصول بايتات من الشبكة، تطلق النواة إشعاراً ليلتقطه الـ Event Loop وينفذ الـ Callback بسرعة ويعود للاستماع فوراً.",
    ...
    [{ title: "The C10K problem", url: "https://en.wikipedia.org/wiki/C10k_problem" }]
  ),
  q(
    "fcnc-009",
    "concurrency-producer-consumer-bounded-buffer",
    "fund-concurrency",
    "Mid",
    "كيف تصمم نمط المنتج والمستهلك (Producer-Consumer) باستخدام حاجز مقيد (Bounded Buffer) مع الضغط العكسي (Backpressure)؟",
    "باستخدام طابور ذي سعة قصوى مدعوم بمتغيرات الشروط (Condition Variables)؛ يتوقف المنتج عن العمل عندما يمتلئ الطابور، ويتوقف المستهلك عندما يفرغ الطابور.",
    ...
    [{ title: "Producer–consumer problem - Wikipedia", url: "https://en.wikipedia.org/wiki/Producer%E2%80%93consumer_problem" }]
  ),
  q(
    "fcnc-010",
    ...
    "fund-concurrency",
    "Senior",
    "قارن بين القفل التفاؤلي (Optimistic Concurrency) والقفل التشاؤمي (Pessimistic Concurrency) في قواعد البيانات والذاكرة؟",
    "القفل التشاؤمي يحجز المورد فوراً ويمنع الآخرين من القراءة أو التعديل حتى انتهاء المعاملة، بينما القفل التفاؤلي يسمح للجميع بالتعديل المتزامن مع فحص إصدار السجل (Version) عند الحفظ ورفض المعاملة إذا تغير.",
    "القفل التشاؤمي (مثل SELECT ... FOR UPDATE) ممتاز عندما تكون احتمالية التعارض عالية جداً (High Contention) مثل حجز المقعد الأخير في طائرة أو شراء سلعة محدودة المخزون؛ لأنه يضمن عدم إهدار وقت المعالجة. أما القفل التفاؤلي فمثالي عندما يكون التعارض نادراً (Low Contention) والقراءة أكثر بكثير من الكتابة؛ حيث نقوم بتحديث السجل بشرط WHERE version = @currentVersion مع زيادة العداد. إذا كان الصف قد عُدل بالفعل، ترجع العملية 0 صفوف متأثرة فنطلب من المستخدم إعادة المحاولة دون أي حجز أو أقفال قاعدة بيانات باهظة.",
    "// قفل تفاؤلي عبر رقم الإصدار في SQL:\nUPDATE products\nSET stock = stock - 1, version = version + 1\nWHERE id = 42 AND version = 5;\n// إذا كانت النتيجة 0 rows، فهذا يعني أن خيطاً آخر قد عدل المخزون قبلك!",
    [
    ...
    [{ title: "Optimistic concurrency control - Wikipedia", url: "https://en.wikipedia.org/wiki/Optimistic_concurrency_control" }]
  )
);
...
// ==========================================
// 8. System Design & Architecture (fsys)
// ==========================================
questions.push(
  q(
    "fsys-001",
    "system-design-horizontal-vs-vertical-scaling-stateless",
    "fund-system-design",
    "Junior",
    "ما الفرق بين التوسع الأفقي (Horizontal Scaling) والتوسع الرأسي (Vertical Scaling) ولماذا تعتبر الخوادم عديمة الحالة (Stateless) شرطاً أساسياً؟",
    "التوسع الرأسي (Scale Up) يعني زيادة موارد نفس السيرفر (RAM, CPU)، بينما التوسع الأفقي (Scale Out) يعني إضافة المزيد من الخوادم المتوازية خلف موزع أحمال (Load Balancer). وتعتبر الخوادم عديمة الحالة شرطاً للتوسع الأفقي لتمكين توجيه أي طلب لأي خادم عشوائياً.",
    "التوسع الرأسي له سقف عتادي صارم (Hardware Limit) وتكلفة متزايدة بشكل أسي ومخاطرة وجود نقطة فشل مفردة (Single Point of Failure). التوسع الأفقي غير محدود عملياً ويمكنه التكيف التلقائي مع الحمل (Auto-scaling). لكن إذا كان السيرفر يخزن جلسة المستخدم (User Session) في ذاكرته المحلية، فإن توجيه الطلب التالي لسيرفر آخر يسجل خروج المستخدم؛ لذا يتم نقل الجلسات إلى كاش مركزي مشترك (مثل Redis) لتصبح الخوادم عديمة الحالة بالكامل (Stateless).",
    "// بنية خوادم عديمة الحالة (Stateless Architecture):\n// [Clients] -> [Load Balancer] -> [App Server 1 | App Server 2 | App Server 3]\n//                                        │                │                │\n//                                        └────────┬───────┴────────┬───────┘\n//                                                 ▼                ▼\n//                                           [Redis Cache]   [Postgres DB]",
    [
    ...
    [{ title: "Scalability - Wikipedia", url: "https://en.wikipedia.org/wiki/Scalability" }]
  ),
  q(
    "fsys-002",
    "system-design-cap-theorem-and-pacelc",
    "fund-system-design",
    "Senior",
    "اشرح نظرية CAP ونظرية PACELC وما الذي تختاره الأنظمة الموزعة عند حدوث انقطاع في الشبكة (Network Partition)؟",
    "تنص نظرية CAP على أنه في أي نظام موزع عند حدوث انقسام في الشبكة (Partition - P)، يستحيل الجمع بين الاتساق الكامل (Consistency - C) والتوافر التام (Availability - A)؛ فيجب التضحية بأحدهما.",
    "إذا انقطع الاتصال بين مركز بيانات في أوروبا وآخر في أمريكا (Network Partition): إذا اخترت الاتساق (CP System مثل HBase أو Zookeeper)، فإن النظام يرفض كتابات جديدة أو يعيد خطأ حتى تعود الشبكة منعاً للتعارض. إذا اخترت التوافر (AP System مثل Cassandra أو DynamoDB)، يتيح النظام القراءة والكتابة في كلا المركزين ولكن يضحي بالاتساق اللحظي معتمداً على الاتساق النهائي (Eventual Consistency). نظرية PACELC توسع ذلك: إذا كان هناك انقسام (P) اختر بين A و C، وإلا (Else - E) اختر بين زمن الاستجابة (Latency - L) والاتساق (Consistency - C).",
    "// الاختيار أثناء انقطاع الشبكة:\n// CP: ضمان صحة الرصيد البنكي حتى لو توقفت الخدمة مؤقتاً\n// AP: بقاء التغريدات ومنشورات السوشيال ميديا متاحة حتى لو تأخرت بضع ثوان في المزامنة",
    [
  ...
  q(
    ...
    [{ title: "Cache-aside pattern - Microsoft Learn", url: "https://learn.microsoft.com/en-us/azure/architecture/patterns/cache-aside" }]
  ),
  q(
    "fsys-004",
    "system-design-load-balancing-consistent-hashing",
    "fund-system-design",
    "Senior",
    "كيف تعمل خوارزمية التجزئة المتسقة (Consistent Hashing) ولماذا تتفوق على خوارزمية Modulo التقليدية في توزيع الأحمال والكاش؟",
    "التجزئة المتسقة ترتب الخوادم والمفاتيح في حلقة دائرية افتراضية، وعند إضافة أو إزالة سيرفر، يُعاد تعيين نسبة طفيفة جداً من المفاتيح (K/N) بدلاً من إعادة توزيع كافة البيانات كما يحدث في خوارزمية Modulo.",
    "في خوارزمية hash(key) % N التقليدية، إذا كان لديك 4 خوادم وأضفت خادماً خامساً ليصبح N=5، فإن نتيجة العملية الحسابية تتغير لـ 99% من المفاتيح تقريباً، مما يتسبب في إفراغ كامل للكاش واختفاء البيانات (Massive Cache Invalidation) وانهيار قواعد البيانات تحت الضغط. تحل Consistent Hashing ذلك برسم حلقة من 0 إلى 2^32 - 1. يوضع السيرفر على الحلقة، ويتم البحث عن أول سيرفر يقع في اتجاه عقارب الساعة لكل مفتاح. تُستخدم العقد الافتراضية (Virtual Nodes) لضمان التوزيع العادل والموحد للأحمال وتفادي البقع الساخنة (Hotspots).",
    "// حلقة التجزئة المتسقة (Consistent Hash Ring):\n// Key K يُوجه لأول سيرفر يليه باتجاه عقارب الساعة في الحلقة الدائرية\n// إضافة خادم جديد تأخذ فقط شريحة صغيرة من الجار المباشر دون المساس ببقية السيرفرات",
    [
  ...
  q(
    ...
    [{ title: "Shard (database architecture) - Wikipedia", url: "https://en.wikipedia.org/wiki/Shard_(database_architecture)" }]
  ),
  q(
    "fsys-006",
    "system-design-relational-vs-nosql-acid-base",
    "fund-system-design",
    "Junior",
    "قارن بين قواعد البيانات العلائقية (SQL) وغير العلائقية (NoSQL) من حيث معايير ACID مقابل BASE؟",
    "قواعد SQL تضمن معايير ACID الصارمة وتتميز بجداول منظمة وعلاقات قوية، بينما تعتمد NoSQL على نموذج BASE وتضحي بالاتساق الفوري لصالح التوسع الأفقي الهائل والمرونة في شكل البيانات.",
    "معايير ACID: الذرية (Atomicity)، الاتساق (Consistency)، العزل (Isolation)، والمتانة (Durability) وهي ضرورية للمعاملات المالية والحسابية. أما نموذج BASE فيعتمد على: التوافر الأساسي (Basically Available)، الحالة اللينة المرنة (Soft state)، والاتساق النهائي (Eventual consistency)، وهو مثالي لأنظمة التخزين الضخمة، تحليل السجلات (Log Analytics)، والبيانات غير المهيكلة (كالملفات الوثائقية في MongoDB ومخازن المفاتيح في Redis وقواعد البيانات العمودية في Cassandra).",
    "// SQL: التزام صارم بالـ Schema ومفتاح أجنبي وضمانات المعاملات\n// NoSQL: مستندات JSON ديناميكية، سهولة تقسيم أفقية، وتحديثات غير مقيدة",
    [
    ...
    [{ title: "ACID vs BASE - Wikipedia", url: "https://en.wikipedia.org/wiki/Eventual_consistency" }]
  ),
  q(
    "fsys-007",
    "system-design-message-queues-kafka-vs-rabbitmq",
    "fund-system-design",
    "Mid",
    "قارن بين وسيط الرسائل المعتمد على الطوابير (RabbitMQ) وسجل الالتزام الموزع (Apache Kafka) ومتى تستخدم كلاً منهما؟",
    "يتميز RabbitMQ بتوجيه الرسائل المعقد والمرن (Smart Broker, Dumb Consumer) وحذف الرسالة بمجرد معالجتها، بينما يعتبر Kafka سجل أحداث متسلسلاً فائق السرعة يحتفظ بالرسائل لفترات طويلة وتتتبع جهات الاستهلاك موضعها بنفسها (Dumb Broker, Smart Consumer).",
    "RabbitMQ ممتاز لمهام الخلفية المتفرقة ومعالجة الطلبات غير المتزامنة التي تتطلب توجيهاً دقيقاً (Routing Keys, Topic Exchanges) وأولويات للرسائل. في المقابل، Kafka مصمم لتدفق البيانات الضخمة (Event Streaming) وملايين الأحداث في الثانية؛ حيث تُكتب الأحداث في أقسام (Partitions) متسلسلة على القرص، ويستطيع المشتركون قراءة الرسائل أو إعادة تشغيلها من البداية (Replay events) في أي وقت دون أن تُحذف.",
    "// RabbitMQ: مهام فردية، إقرارات لحظية، وحذف فوري للرسالة المستهلكة\n// Kafka: سجل أحداث ثابت غير قابل للتعديل (Append-Only Log) يدعم القراءة المكررة وملايين الرسائل/ثانية",
    [
    ...
    [{ title: "Apache Kafka Architecture", url: "https://en.wikipedia.org/wiki/Apache_Kafka" }]
  ),
  q(
    "fsys-008",
    ...
    "fund-system-design",
    "Senior",
    "كيف تعمل خوارزميات تحديد معدل الطلبات (Rate Limiting): خوارزمية Token Bucket وخوارزمية Leaky Bucket؟",
    "تعتمد Token Bucket على إضافة رموز إلى دلو بمعدل ثابت وتسمح برشقات من الطلبات طالما توفرت رموز، بينما تمرر Leaky Bucket الطلبات بمعدل تدفق ثابت تماماً كقطرات الماء دون السماح بأي طفرات مفاجئة.",
    "في خوارزمية Token Bucket، يمتلك الدلو سعة قصوى C، وتضاف الرموز بمعدل R رمز/ثانية. مع كل طلب، يُخصم رمز واحد؛ إذا فرغ الدلو يُرفض الطلب بخطأ 429 Too Many Requests. ميزتها أنها تسمح بمرور رشقة سريعة (Burst) طالما لم يتجاوز الحجم سعة الدلو. أما خوارزمية Leaky Bucket فتضع الطلبات في طابور وتخرجها بمعدل خروج ثابت وصارم، مما يضمن حملاً منتظماً تماماً على الأنظمة الحساسة التي لا تتحمل أي قفزات مفاجئة.",
    "// خوارزمية Token Bucket في Redis:\n// يتم التحقق من الوقت المنقضي وإضافة الرموز الحسابية رياضياً دون الحاجة لمؤقتات خلفية مستمرة!",
    [
  ...
  q(
    "fsys-009",
    "system-design-circuit-breaker-resiliency-pattern",
    "fund-system-design",
    "Mid",
    "كيف يحمي نمط قاطع الدائرة (Circuit Breaker) الأنظمة الموزعة من الانهيار المتسلسل (Cascading Failure)؟",
    "عبر مراقبة إخفاقات الخدمة التابعة وقطع الاتصال بها فوراً (Open State) بمجرد تجاوز عتبة الخطأ، للرد المباشر ببدائل سريعة دون تعليق موارد وسلاسل خيوط الخادم في طلبات فاشلة.",
    "إذا توقفت خدمة المدفوعات الخارجية، فإن كل طلب مستخدم يعلق لانتظار الـ Timeout (مثلاً 30 ثانية). خلال دقائق، تنفد جميع خيوط الاتصال بالسيرفر ويتوقف النظام بأكمله عن الاستجابة لجميع المستخدمين. يعمل Circuit Breaker بثلاث حالات: (1) مغلق (Closed): الطلبات تمر طبيعياً ويتم حساب نسبة الفشل. (2) مفتوح (Open): إذا تجاوزت الأخطاء 50%، يفتح القاطع ويمنع إرسال أي طلب للخادم المنهار ويرجع Fallback فوري. (3) نصف مفتوح (Half-Open): بعد انتهاء فترة تجريبية، يسمح بمرور بضعة طلبات؛ إذا نجحت يغلق القاطع، وإن فشلت يعود لحالة الفتح.",
    ...
    [{ title: "Circuit Breaker pattern - Martin Fowler", url: "https://martinfowler.com/bliki/CircuitBreaker.html" }]
  ),
  q(
    "fsys-010",
    "system-design-saga-pattern-vs-two-phase-commit",
    "fund-system-design",
    "Senior",
    "كيف تدير المعاملات الموزعة في بنية المايكروسيرفس وما الفرق بين Two-Phase Commit ونمط Saga؟",
    "بروتوكول 2PC يفرض قفلاً موزعاً متزامناً عبر منسق مركزي يضمن الاتساق الصارم لكنه يعاني من بطء شديد ونقطة فشل مفردة، بينما يدير نمط Saga المعاملات كسلسلة من المعاملات المحلية المنفصلة مع معاملات تعويضية (Compensating Transactions) للتراجع عند الفشل.",
    "في نمط Saga، تنفذ كل خدمة معاملتها المحلية وتطلق حدثاً يخبر الخدمة التالية لإكمال دورها. إذا فشلت خطوة في المنتصف (مثل رفض بطاقة الائتمان في خدمة الدفع بعد حجز المقعد في خدمة التذاكر)، تطلق الخدمة حدث فشل وتبدأ خطة التراجع بتنفيذ معاملات تعويضية بالعكس (إلغاء حجز المقعد). ينقسم Saga إلى: Choreography (حيث تتواصل الخدمات ذاتياً عبر الأحداث) أو Orchestration (حيث يوجه منسق مركزي الأوامر والتعويضات).",
    ...
    [{ title: "Saga pattern - Microsoft Learn", url: "https://learn.microsoft.com/en-us/azure/architecture/patterns/saga" }]
  )
);

// ==========================================
// 9. Computer Networks & Protocols (fnet)
// ==========================================
questions.push(
  q(
    "fnet-001",
    "networking-osi-vs-tcpip-model-encapsulation",
    "fund-networking",
    "Junior",
    "قارن بين نموذج OSI المكون من 7 طبقات ونموذج TCP/IP وكيف تعمل عملية التغليف (Encapsulation) لحزم البيانات؟",
    "نموذج OSI هو إطار نظري مرجعي من 7 طبقات، بينما TCP/IP هو النموذج العملي الحقيقي للإنترنت المكون من 4 طبقات؛ وتغليف البيانات يعني إضافة ترويسة (Header) خاصة بكل طبقة أثناء هبوط البيانات للأسفل.",
    "طبقات OSI: التطبيق (Application)، العرض (Presentation)، الجلسة (Session)، النقل (Transport)، الشبكة (Network)، ربط البيانات (Data Link)، والفيزيائية (Physical). في TCP/IP، دُمجت الطبقات الثلاث العليا في طبقة التطبيق. عند إرسال طلب، تضاف ترويسة TCP في طبقة النقل (لتصبح Segment)، ثم ترويسة IP في طبقة الشبكة (لتصبح Packet)، ثم ترويسة Ethernet في طبقة الرابط (لتصبح Frame)، ثم تُحول لإشارات كهربائية/ضوئية (Bits) عبر الأسلاك.",
    "// تدفق التغليف (Encapsulation):\n// [Data] -> [TCP | Data] -> [IP | TCP | Data] -> [Ethernet | IP | TCP | Data | FCS]",
    [
  ...
  q(
    "fnet-002",
    "networking-tcp-vs-udp-handshake-reliability",
    "fund-networking",
    "Junior",
    "قارن بين بروتوكول TCP وبروتوكول UDP وكيف تضمن مصافحة الـ 3-Way Handshake موثوقية الاتصال؟",
    "بروتوكول TCP موثوق، موجه بالاتصال (Connection-oriented)، يضمن وصول وترتيب الحزم والتحكم بالتدفق، بينما UDP بروتوكول عديم الاتصال وسريع جداً دون أي ضمان للوصول أو الترتيب.",
    "تؤسس مصافحة TCP الثلاثية الاتصال عبر: (1) إرسال العميل حزمة SYN برقم تسلسلي مبدئي. (2) رد الخادم بـ SYN-ACK لتأكيد الاستلام وإرسال رقمه الخاص. (3) رد العميل بـ ACK لبدء نقل البيانات. يعيد TCP إرسال الحزم المفقودة تلقائياً (Retransmission) ويتحكم بالازدحام (Congestion Control). في المقابل، يرسل UDP الحزم فوراً دون انتظار، مما يجعله الخيار الأمثل للبث المباشر (Video Streaming)، ألعاب الفيديو التنافسية، واستعلامات DNS السريعة.",
  ...
  q(
    ...
    [{ title: "Transport Layer Security - Wikipedia", url: "https://en.wikipedia.org/wiki/Transport_Layer_Security" }]
  ),
  q(
    "fnet-005",
    "networking-dns-resolution-recursive-iterative",
    "fund-networking",
    "Junior",
    "كيف تجري دورة حياة دقة نظام أسماء النطاقات (DNS Resolution) وما الفرق بين الاستعلامات المتكررة والمتتالية؟",
    "يقوم DNS بترجمة أسماء النطاقات البشرية (example.com) إلى عناوين IP رقمية؛ في الاستعلام التكراري (Recursive) يتولى الخادم الوسيط جلب الإجابة الكاملة للعميل، بينما في الاستعلام المتتالي (Iterative) يوجه الخادم السائل إلى الخادم التالي في التسلسل الهرمي.",
    "عند كتابة رابط في المتصفح: (1) فحص الكاش المحلي في المتصفح ثم نظام التشغيل ثم الروتر. (2) إرسال استعلام لمحلل DNS لمزود الإنترنت (DNS Resolver). (3) يستعلم المحلل خوادم الجذر (Root Servers) التي توجهه لخوادم نطاق المستوى الأعلى (.com TLD Servers). (4) يوجه خادم TLD المحلل إلى خادم الأسماء المعتمد للنطاق (Authoritative Name Server). (5) يجلب المحلل سجل A ويحفظه في الكاش وفقاً لـ TTL ويرسله للمتصفح ليبدأ الاتصال.",
    "// مسار دقة الـ DNS الهرمي:\n// Browser -> Local Cache -> DNS Resolver -> Root (.) -> TLD (.com) -> Authoritative (example.com) -> IP",
    [
    ...
    [{ title: "Domain Name System - Wikipedia", url: "https://en.wikipedia.org/wiki/Domain_Name_System" }]
  ),
  q(
    "fnet-006",
    "networking-websockets-vs-long-polling-sse",
    "fund-networking",
    "Mid",
    "قارن بين تقنيات الاتصال اللحظي: WebSockets و Server-Sent Events (SSE) و Long Polling ومتى تختار كلاً منها؟",
    "الـ WebSockets يوفر اتصالاً ثنائي الاتجاه كاملاً (Full-Duplex) فوق اتصال TCP واحد مستمر، والـ SSE يوفر تدفقاً أحادي الاتجاه من الخادم للعميل فوق HTTP القياسي، والـ Long Polling يعتمد على إبقاء طلبات HTTP معلقة حتى تتوفر بيانات جديدة.",
    "إذا كان التطبيق يتطلب تواصلاً سريعاً في الاتجاهين (مثل غرف الدردشة الجماعية، الألعاب متعددة اللاعبين، والبورصات اللحظية)، فالـ WebSockets هو الخيار الأفضل لأنه يزيل ترويسات HTTP المتكررة. إذا كان التطبيق يتطلب بث بيانات من الخادم فقط دون إرسال من العميل (مثل أسعار العملات، إشعارات التطبيق، وتدفق نصوص ChatGPT)، فالـ SSE ممتاز وبسيط للغاية ويدعم إعادة الاتصال التلقائي ومرور الجدران النارية والبروكسيات بسهولة عبر HTTP.",
    "// WebSockets: ثنائي الاتجاه (Client <───> Server) عبر ws:// أو wss://\n// SSE: أحادي الاتجاه (Server ───> Client) عبر Content-Type: text/event-stream\n// Long Polling: طلبات متتالية تنتظر الرد وتفتح طلباً جديداً فوراً",
    [
    ...
    [{ title: "WebSockets API - MDN Web Docs", url: "https://developer.mozilla.org/en-US/docs/Web/API/WebSockets_API" }]
  ),
  q(
    "fnet-007",
    "networking-rest-api-principles-idempotency",
    "fund-networking",
    "Junior",
    "ما هي مبادئ معمارية REST وما معنى أن تكون العملية متكررة النتائج (Idempotent)؟",
    "تعتمد REST على بنية عميل/خادم عديمة الحالة (Stateless)، وموارد محددة بروابط URI موحدة، واستخدام أفعال HTTP القياسية؛ وتكون العملية Idempotent إذا كان تكرار تنفيذها N مرة ينتج نفس الحالة النهائية للنظام تماماً كمرة واحدة.",
    "أفعال GET و PUT و DELETE و HEAD و OPTIONS كلها Idempotent؛ فإذا حذفت المورد DELETE /users/42 عشر مرات، فإن النتيجة النهائية على قاعدة البيانات هي حذف المستخدم ولا يختلف أثره في المرة الثانية عن الأولى. بينما يعتبر POST غير متكرر النتائج (Non-Idempotent)؛ لأن إرساله مرتين سينشئ سجلين أو معاملتين ماليتين مكررتين ما لم يُحمى بمفتاح عدم تكرار (Idempotency Key).",
    "// GET, PUT, DELETE: Idempotent (آمن للتكرار عند انقطاع الشبكة)\n// POST: Non-Idempotent (يتطلب حماية مفتاح idempotency-key)",
    [
  ...
  q(
    "fnet-008",
    "networking-cors-preflight-options-headers",
    "fund-networking",
    "Junior",
    "ما هي آلية مشاركة الموارد عبر الأصول (CORS) ولماذا يرسل المتصفح طلب Preflight من نوع OPTIONS تلقائياً؟",
    "الـ CORS هي آلية أمان تطبقها المتصفحات لمنع مواقع خبيثة من قراءة بيانات موقعك، ويرسل المتصفح طلب Preflight استطلاعي بنوع OPTIONS للتأكد من موافقة السيرفر قبل إرسال الطلبات المعدلة أو المعقدة.",
    "سياسة الأصل الواحد (Same-Origin Policy - SOP) تمنع السكريبت من نطاق foo.com من قراءة استجابة api.bar.com. إذا كان الطلب معقداً (يستخدم PUT/DELETE، أو ترويسات مخصصة مثل Authorization، أو Content-Type غير تقليدي مثل application/json)، يرسل المتصفح أولاً طلب OPTIONS يحتوي على Access-Control-Request-Method. إذا رد الخادم بترويسات قبول متوافقة (Access-Control-Allow-Origin)، يرسل المتصفح الطلب الفعلي، وإلا يمنعه ويرمي خطأ في الكونسول.",
    "// استجابة السيرفر المسموح بها:\nAccess-Control-Allow-Origin: https://myapp.com\nAccess-Control-Allow-Methods: GET, POST, PUT, DELETE\nAccess-Control-Allow-Headers: Authorization, Content-Type",
    [
    ...
    [{ title: "Cross-Origin Resource Sharing (CORS) - MDN", url: "https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS" }]
  ),
  q(
    "fnet-009",
    "networking-security-mitm-syn-flood-ddos",
    "fund-networking",
    "Senior",
    "كيف تحدث هجمات الرجل في المنتصف (MITM) وهجمات إغراق المزامنة (SYN Flood) وكيف تعمل تقنية SYN Cookies؟",
    "تحدث هجمات MITM عند اعتراض وتعديل البيانات بين طرفين دون علمهما ويحميها التشفير والشهادات الموثوقة، وهجمات SYN Flood تغرق الخادم بطلبات اتصال وهمية لملء طابور الاتصالات واستنزاف موارده.",
    "في هجوم SYN Flood، يرسل المهاجم ملايين حزم SYN بعناوين IP وهمية (Spoofed IPs)، فيرد الخادم بـ SYN-ACK وينتظر رد ACK النهائي واضعاً الاتصال في طابور نصف المكتمل (SYN Queue / Backlog). تمتلئ ذاكرة الخادم ويتوقف عن استقبال أي مستخدم حقيقي. تعالج تقنية SYN Cookies ذلك بعبقرية: لا يخصص الخادم أي ذاكرة على الإطلاق في البداية! بل يشفر بيانات الاتصال والوقت في الرقم التسلسلي المبدئي (Initial Sequence Number)، وعندما يرد العميل الشرعي بـ ACK، يتحقق الخادم من صحة التشفير حسابياً ويفتح الاتصال فوراً.",
    "// SYN Cookies تشفر حالة الاتصال رياضياً داخل Sequence Number بدلاً من حجز ذاكرة الخادم",
    [
    ...
    [{ title: "SYN cookies - Wikipedia", url: "https://en.wikipedia.org/wiki/SYN_cookies" }]
  ),
  q(
    "fnet-010",
    "networking-cdn-edge-caching-anycast-routing",
    "fund-networking",
    "Mid",
    "كيف تعمل شبكات توصيل المحتوى (CDNs) وكيف توجه تقنية Anycast Routing المستخدمين لأقرب خادم حافة (Edge Server)؟",
    "توزع الـ CDNs المحتوى الثابت والوسائط جغرافياً على مئات الخوادم المحيطة بالمستخدمين؛ وتعلن تقنية Anycast عن نفس عنوان الـ IP من عشرات المواقع عالمياً ليوجه بروتوكول BGP الزائر لأقرب نقطة بأقل زمن تأخير.",
    "في التوجيه التقليدي (Unicast)، يملك السيرفر عنوان IP فريد في مكان واحد، مما يعني أن مستخدم اليابان سيعاني من تأخير 250 مللي ثانية للوصول لخادم في نيويورك. باستخدام Anycast وشبكات CDN (مثل Cloudflare و Fastly)، يعلن كل خادم حافة محلي عن نفس الـ IP، فتقوم راوترات الإنترنت بتوجيه حزم المستخدم لأقرب سيرفر على بعد بضعة كيلومترات (Edge Node) في 5 مللي ثانية فقط، مما يخفف الحمل عن الخادم الأصلي (Origin) بنسبة 90% ويوفر حماية ضخمة ضد هجمات الـ DDoS.",
    "// المستخدم يطلب الصورة -> يتصل بـ Anycast Edge محلي في ثوان -> إن كانت في الكاش تُسلم فوراً\n// إن لم تكن في الكاش -> يجلبها الحافة من الـ Origin مرة واحدة ويخزنها للبقية",
    [
    ...
    [{ title: "Content delivery network - Wikipedia", url: "https://en.wikipedia.org/wiki/Content_delivery_network" }]
  )
);

// ==========================================
// 10. Software Testing & Quality (ftst)
// ==========================================
questions.push(
  q(
    "ftst-001",
    "testing-pyramid-unit-integration-e2e-cost-ratio",
    "fund-testing",
    "Junior",
    "ما هو هرم الاختبارات (The Test Pyramid) وما هو التوازن المثالي بين اختبارات الوحدة، التكامل، والاختبارات الشاملة (E2E)؟",
    "يقترح هرم الاختبارات قاعدة عريضة من اختبارات الوحدة السريعة والرخيصة في الأسفل، تليها اختبارات التكامل في المنتصف، وقمة مدببة من الاختبارات الشاملة (E2E) المعقدة والبطيئة.",
    "اختبارات الوحدة (Unit Tests) تختبر دوال وفئات معزولة بالميكروثانية وتمنح تشخيصاً لحظياً دقيقاً لمكان الخطأ ورخيصة الصيانة. اختبارات التكامل (Integration Tests) تتحقق من سلامة تفاعل عدة مكونات حقيقية معاً (مثل الخدمة مع قاعدة البيانات). أما اختبارات E2E فتحاكي المستخدم الحقيقي في المتصفح؛ وهي ممتازة لزرع الثقة قبل الشحن ولكنها بطيئة جداً ومكلفة وعرضة للاهتزاز (Flakiness). الاعتماد على هرم مقلوب (Ice Cream Cone Anti-pattern) يقتل سرعة فرق التطوير.",
  ...
  q(
    ...
    "fund-testing",
    "Junior",
    "ما هي الدورة الثلاثية للتطوير الموجه بالاختبارات (TDD: Red-Green-Refactor) وما هي ميزتها التصميمية وليست الاختبارية فقط؟",
    "تبدأ بكتابة اختبار صغير يفشل أولاً (Red)، ثم كتابة أقل كود كافٍ لنجاح الاختبار (Green)، ثم إعادة تحسين وهيكلة الكود وتنظيفه مع ضمان بقاء الاختبار ناجحاً (Refactor).",
    "الميزة الكبرى لـ TDD ليست مجرد فحص الأخطاء، بل هي أداة تصميم هندسي (Design Feedback Tool). عندما تكتب الاختبار قبل الكود، فإنك تجبر نفسك على التفكير كمستخدم للواجهة البرمجية (API Consumer) وليس كصانع لها، مما ينتج واجهات بديهية وسهلة الاستخدام ووحدات مفكوكة الارتباط بطبيعتها وقابلة للاختبار (Testable by design).",
    "// 1. Red: اكتب اختباراً يستدعي دالة غير موجودة بعد -> يفشل\n// 2. Green: اكتب الدالة بأبسط شكل ممكن حتى يمر الاختبار\n// 3. Refactor: أزل التكرار وحسن التسميات ونظم الكود مع الحفاظ على اللون الأخضر",
    [
  ...
  q(
    "ftst-003",
    "testing-doubles-taxonomy-dummy-stub-fake-mock-spy",
    "fund-testing",
    "Mid",
    "اشرح تصنيف بدلاء الاختبار (Test Doubles) بحسب جيرارد ميسزاروس: ما الفرق بين Dummy و Stub و Fake و Mock و Spy؟",
    "البدلاء هم كائنات تُستبدل في الاختبار بالتبعية الحقيقية: الـ Dummy لملء البارامترات فقط، والـ Stub يعيد إجابات ثابتة معدة مسبقاً، والـ Spy يسجل معلومات الاستدعاءات، والـ Mock يضع توقعات ويتحقق من السلوك، والـ Fake تطبيق حقيقي خفيف كقاعدة بيانات في الذاكرة.",
    "الـ Dummy مثل كائن فارغ يُمرر لدالة تطلب 3 معاملات وأنت تحتاج واحداً فقط. الـ Stub يعيد دوماً true عند استدعاء isValid(). الـ Spy يراقب ويسجل: كم مرة استُدعيت دالة sendEmail() وما هي المعاملات التي مُررت لها. الـ Mock يحدد الشروط مسبقاً: 'أتوقع استدعاء save() مرة واحدة بالضبط'، ويفشل الاختبار إذا لم يُستدع. أما الـ Fake فهو تنفيذ يعمل بالكامل لكنه مبسط ولا يناسب الإنتاج، مثل فئة InMemoryUserRepository.",
    "// Fake: تطبيق شغال بالكامل في الذاكرة بدون قاعدة بيانات حقيقية\nclass FakeUserRepository implements UserRepository {\n  private users = new Map<string, User>();\n  async save(u: User) { this.users.set(u.id, u); }\n  async findById(id: string) { return this.users.get(id) ?? null; }\n}",
    [
  ...
  q(
    ...
    [{ title: "F.I.R.S.T principles of testing", url: "https://en.wikipedia.org/wiki/Unit_testing" }]
  ),
  q(
    "ftst-005",
    "testing-code-coverage-vs-mutation-testing",
    "fund-testing",
    "Senior",
    "لماذا تعتبر نسبة تغطية الكود (Code Coverage) مقياساً مضللاً أحياناً وما هي اختبارات الطفرات (Mutation Testing)؟",
    "تغطية الكود تقيس فقط الأسطر التي مر عليها التنفيذ دون التحقق من جودة وصحة الـ Assertions؛ بينما تقوم اختبارات الطفرات بحقن أخطاء متعمدة في الشفرة وتفحص قدرة الاختبارات على اكتشافها وقتلها (Kill Mutants).",
    ...
    [{ title: "Mutation testing - Wikipedia", url: "https://en.wikipedia.org/wiki/Mutation_testing" }]
  ),
  q(
    "ftst-006",
    "testing-flaky-tests-root-causes-mitigation",
    "fund-testing",
    "Mid",
    "ما هي الاختبارات المتقلبة (Flaky Tests) وما هي أسبابها الجذرية وكيف تقضي عليها في بيئات الـ CI/CD؟",
    "هي الاختبارات التي تنجح وتفشل عشوائياً لنفس الكود دون أي تغيير؛ وتنتج عادة عن الاعتماد على توقيتات غير متزامنة، حالات مشتركة بين الاختبارات، أو تقلبات الاتصال الخارجي والشبكة.",
    "تعتبر الاختبارات المتقلبة سماً قاتلاً لثقة الفريق؛ لأن المطورين يبدأون في تجاهل فشل الـ CI والضغط على زر 'Re-run' حتى يمر الاختبار صدفة. الأسباب وعلاجها: (1) استخدام sleep/delay زمني ثابت بدلاً من انتظار الشروط صراحة (Await Polling / waitFor). (2) بقاء بيانات في الذاكرة أو قاعدة البيانات بين الاختبارات وعلاجها بالتنظيف التام بعد كل اختبار. (3) التبعيات على خدمات خارجية واستبدالها بـ Mocks ومحاكيات محلية (مثل WireMock / Testcontainers).",
    ...
    [{ title: "Flaky tests in software engineering", url: "https://en.wikipedia.org/wiki/Flaky_test" }]
  ),
  q(
    "ftst-007",
    "testing-property-based-testing-vs-example-based",
    "fund-testing",
    "Senior",
    "ما هو الاختبار القائم على الخصائص (Property-Based Testing) وكيف يكشف حالات الحافة المجهولة مقارنة بالاختبار القائم على الأمثلة؟",
    "بدلاً من فحص أمثلة ومدخلات محددة يكتبها المبرمج يدوياً، يحدد المبرمج خصائص عامة صادقة دائماً (Invariants) وتقوم أداة الاختبار بتوليد مئات الآلاف من المدخلات العشوائية والمتطرفة لاكتشاف أي كسر لتلك الخصائص.",
    "في Example-based Testing، تفحص: sort([3, 1, 2]) == [1, 2, 3]. في Property-based Testing (مثل مكتبات QuickCheck أو Fast-Check)، نحدد الخصائص: (1) طول المصفوفة المرتبة يجب أن يطابق المصفوفة الأصلية. (2) كل عنصر يجب أن يكون <= العنصر التالي له. (3) كل عنصر في المصفوفة الأصلية موجود في الناتجة. تقوم الأداة بتوليد مصفوفات فارغة، سالبة، ضخمة، أو مكررة، وإذا وجدت خطأً، تقوم بتقليص المدخلات (Shrinking) للوصول لأصغر مدخل يسبب الانهيار.",
    "// خاصية الانعكاس في فك وضغط البيانات (Roundtrip property):\n// forall x: decompress(compress(x)) === x",
    [
    ...
    [{ title: "Property-based testing - Wikipedia", url: "https://en.wikipedia.org/wiki/QuickCheck" }]
  ),
  q(
    "ftst-008",
    "testing-bdd-given-when-then-ubiquitous-language",
    "fund-testing",
    "Junior",
    "ما هو التطوير الموجه بالسلوك (Behavior-Driven Development - BDD) وصيغة Given-When-Then؟",
    "هو تطور لـ TDD يهدف لتوحيد لغة التواصل بين المطورين ومحللي الأعمال باستخدام لغة دومين موحدة (Ubiquitous Language)، مع صياغة سيناريوهات الاختبار بصيغة Given (السياق) و When (الحدث) و Then (النتيجة المتوقعة).",
    "تساعد صيغة Given-When-Then في جعل مواصفات الاختبار مفهومة لجميع الأطراف المعنية (مثل استخدام ملفات Gherkin في أدوات Cucumber). تصف Given الحالة الابتدائية للنظام (المستخدم مسجل دخول ورصيده 100)، وتصف When الإجراء الذي اتخذه المستخدم (قام بسحب 40)، وتصف Then الحالة النهائية للنظام (يصبح رصيده 60 ويتم إصدار إشعار بالسحب). هذا يضمن أن الفريق يبني الشيء الصحيح المطلوب للأعمال.",
    ...
    [{ title: "Behavior-driven development - Wikipedia", url: "https://en.wikipedia.org/wiki/Behavior-driven_development" }]
  ),
  q(
    "ftst-009",
    "testing-contract-testing-microservices-pact",
    "fund-testing",
    "Senior",
    "ما هي اختبارات التعاقد (Contract Testing) وكيف تحمي خدمات المايكروسيرفس الموزعة دون الحاجة لتشغيل بيئات E2E مجمعة؟",
    "تتحقق اختبارات التعاقد من توافق واجهات الـ API بين المستهلك (Consumer) والمزود (Provider) بشكل مستقل ومنفصل دون الحاجة لتشغيل النظام الموزع بالكامل في بيئة اختبار موحدة.",
    "في معمارية المايكروسيرفس، تشغيل 50 خدمة معاً لاختبارات E2E بطيء جداً وغير مستقر ومكلف. بدلاً من ذلك، في Consumer-Driven Contract Testing (باستخدام أدوات مثل Pact)، يكتب المستهلك اختباراً يحدد ما يتوقعه من المزود (عقد Contract بصيغة JSON). يُرسل هذا العقد لوسيط (Pact Broker)، ثم يقوم المزود بتشغيل اختبار ضد هذا العقد للتحقق من التزامه به. إذا قام المزود بتغيير حقل حاسم يكسر المستهلك، يفشل بناء المزود فوراً قبل النشر دون أي حاجة لبيئات متصلة.",
    "// عقد المستهلك: أتوقع من GET /users/1 أن يعيد حقل 'fullName' بنوع string\n// المزود يختبر واجهته ضد هذا العقد ويضمن عدم كسر التطبيقات التابعة له",
    [
    ...
    [{ title: "Pact Contract Testing Documentation", url: "https://en.wikipedia.org/wiki/Contract_testing" }]
  ),
  q(
    "ftst-010",
    "testing-ci-cd-strategies-parallelization-splitting",
    "fund-testing",
    "Mid",
    "كيف تحسن زمن تنفيذ الاختبارات في مسارات التكامل المستمر (CI Pipelines) عبر التوازي وتقسيم الأحمال؟",
    "عبر تقسيم مجموعة الاختبارات الكبيرة إلى حزم متوازية تعمل على عدة خوادم في نفس الوقت (Test Parallelization / Matrix)، وتشغيل الاختبارات المتأثرة فقط بالتغيير الأخير (Test Impact Analysis).",
    "إذا كان جناح الاختبارات يستغرق ساعة كاملة للعمل على خادم واحد، فإن دورة تسليم الكود تصبح بالغة البطء. الحلول الفعالة: (1) تقسيم الاختبارات عبر خوارزميات التوقيت التاريخي (Timing-based Test Splitting) لتوزيع الحزم بالتساوي بين 4 أو 8 عمال (Workers). (2) تشغيل اختبارات التكامل التي تتطلب قواعد بيانات باستخدام حاويات خفيفة معزولة في الذاكرة (tmpfs). (3) عزل واختبار الميزات عبر Test Impact Analysis لتشغيل الاختبارات المرتبطة فقط بالملفات المعدلة في الـ PR.",
    "// تكوين GitHub Actions لتقسيم الاختبارات على 4 أجهزة متوازية:\nstrategy:\n  matrix:\n    ci_node_total: [4]\n    ci_node_index: [0, 1, 2, 3]",
    [
    ...
    [{ title: "Continuous integration - Wikipedia", url: "https://en.wikipedia.org/wiki/Continuous_integration" }]
  )
);

// Write to src/content/fundamentals-questions.ts
const outputFile = resolve(process.cwd(), "src/content/fundamentals-questions.ts");
const content = `// Generated for Software Fundamentals Track (100 in-depth CS questions across 10 topics)
import type { InterviewQuestion } from "./questions.ts";

export const fundamentalsBaseQuestions: Omit<InterviewQuestion, "translations">[] = ${JSON.stringify(questions, null, 2)};
`;

writeFileSync(outputFile, content, "utf8");
console.log(`Generated ${questions.length} Software Fundamentals questions at ${outputFile}`);
