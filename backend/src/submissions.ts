...
export function createSupabaseSubmissionStore({ url, serviceRoleKey, fetchImpl = fetch }: { url: string; serviceRoleKey: string; fetchImpl?: typeof fetch }): SubmissionRouteStore {
  return {
    async submit(draft, accessToken, userId) {
      ...
      const response = await handleSubmit(new Request("http://node.internal/v1/submissions", { method: "POST", headers: { "x-account-id": userId, "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify(draft) }), fetchImpl, { url, key: serviceRoleKey });
