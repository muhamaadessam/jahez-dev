import { validateSubmission } from "../../shared/submissions.ts";
...
export type SubmissionRouteStore = { submit: (draft: unknown, userId: string) => Promise<unknown> };
...
export function createSupabaseSubmissionStore({ url, serviceRoleKey, fetchImpl = fetch }: { url: string; serviceRoleKey: string; fetchImpl?: typeof fetch }): SubmissionRouteStore {
  return {
    async submit(draft, userId) {
      ...
      const response = await handleSubmit(new Request("http://node.internal/v1/submissions", { method: "POST", headers: { "x-account-id": userId, "Content-Type": "application/json" }, body: JSON.stringify(draft) }), fetchImpl, { url, key: serviceRoleKey });
